...
- **Platform işleri eki (kapasite notu) [T]:** Son temizlik turunda şu hikâyeler eklendi: S1-12 i18n altyapısı, S2-15 ürün analitiği v1, S3-16 sürüm yönetimi v1, S4-15 sürüm treni ve öncü halka, S4-16 panel analitiği, S5-16 panel güncelleme e2e testi, S6-21 A-14 "Sürümler". Geliştiricilere düşen ek yük kabaca S1 ≈ 1–2, S2 ≈ 1–2, S3 ≈ 2–3, S4 ≈ 2–3, S5 ≈ 1–2, S6 ≈ 1 geliştirici-günüdür [T] (tahmin; S1 planlamasında hikâye puanıyla teyit edilir). S3 (tatil) ve S5 zaten ~%80 planlı olduğundan önce "S" işaretli analitik hikâyeleri (S2-15, S4-16) ve S6-21 kayar: S2-15/S4-16 → H13 (o zamana kadar pilot panosu cihaz nabzı ve rollup'larla izlenir; sağlık skorunun "panel kullanımı" bileşeni nabızdan geçici olarak türetilir [T]), S6-21 → S7 (satırlar deploy scriptiyle yazılmaya devam eder). S3-16 ve S5-16 P0 öncesi zorunludur ve kesilmez (§5.9).
...
| S1-12 | E0 → i18n metin ayıklama altyapısı **[Faz 1 altyapı · Faz 3 diller]** (`packages/i18n`; [06](06-teknik-mimari.md) §4.3, [12](12-marka-tasarim-ve-kullanilabilirlik.md) §11.2–11.3) (FE) | Kaynak katalog `tr.json`, anahtar biçimi `panel.orders.accept_button`; ICU MessageFormat (FormatJS önerisi [T]) panel, admin ve storefront'ta aynı katalogla çalışır; sayı içeren metin ICU `plural` ile yazılır; metin birleştirme yasak; JSX içindeki çıplak metin lint ile yakalanır (kural adı teyit edilmeli); `formatTL()`, `formatSaat()` ve `tr-TR` büyük/küçük harf yardımcıları pakette. Sonraki sprintlerin tüm UI metinleri bu katalogdan gelir; ICU kapsamının %100'e çıkması ve sözde yerel ayar testi F2-21'dedir | M |
...
| S2-15 | E0 → Ürün analitiği v1: tek olay sözlüğü `packages/core/events.ts`, `analytics_events` tablosu ve `POST /api/v1/store/events` ([07](07-veri-modeli-ve-api.md) §3.5, §6.2; [03](03-musteri-deneyimi-ve-storefront.md) §11) (TL, FE; bağımlılık S2-04) | Storefront huni olayları `sendBeacon` ile toplu gider, yanıt `204`. Sözlük dışı olay ve izin listesi dışı alan atılır; PII kalıbı (telefon, e-posta) taşıyan istek bütünüyle reddedilir; IP ve user agent saklanmaz. Şema lint'i `events.ts`'te `pii:` kayıtlı alan adını yakalar ([07](07-veri-modeli-ve-api.md) §10). `retention.analytics` 90 gün kuralı tabloda tanımlı (işi S6-09). `sessionStorage` oturum kimliğinin rıza gereksinimi avukat teyidine bağlıdır ([07](07-veri-modeli-ve-api.md) §11 #25); olumsuzsa oturum kimliği kaldırılır | S |
...
| S3-16 | E0 → Sürüm yönetimi v1 ve panel güncelleme politikası ([06](06-teknik-mimari.md) §16.7 madde 1–3; bağımlılık S3-05, S3-06) (TL, FE) | `app_releases` tablosu ([07](07-veri-modeli-ve-api.md) §3.7); A-14 "Sürümler" ekranına (S6-21) kadar satır deploy scriptiyle yazılır. Service worker "prompt" modunda, otomatik `skipWaiting` kapalı (davranış teyit edilmeli); güncelleme yalnız (a) güvenli anda (açık `new` sipariş, alarm, bekleyen ret, `busy` ve kaydedilmemiş form yok; yoğun saat dışı) ya da (b) "Vardiya başlat" (P-03) anında uygulanır. API `X-App-Version` / `X-Min-App-Version` döner, nabız `app_version` ve `audio_unlocked` bildirir; asgari sürümün altındaki panel sipariş aksiyonlarında hiçbir koşulda engellenmez. Yeniden yüklemeden sonra ses kilitliyse bant + ilk nabızda Web Push (`audio_locked`) + 5 dk'da `owner`'a platform WhatsApp uyarısı | M |
...
| S4-15 | E0 → Haftalık sürüm treni, hotfix yolu ve öncü halka ([06](06-teknik-mimari.md) §16.7 madde 4–5; bağımlılık S3-16, S4-13) (TL) | Prod'a haftada bir planlı tren, deploy penceresi içinde (gün ve saat önerisi [T]); tren öncesi staging e2e paketi yeşil, elle keşif listesi ([12](12-marka-tasarim-ve-kullanilabilirlik.md) §9.3) koşuldu, "Yenilikler" notu yazıldı. `hotfix/*` yolu tam CI'dan geçer, panel hotfix'i `app_releases.is_hotfix = true`. `channel = 'pilot'` sürümünde nabız yanıtındaki hedef sürüm yalnız halka 0–1 cihazlarına verilir. Pilotun ilk canlı siparişinden (7 Aralık) önce en az bir tren staging'de kuru olarak işletilmiş olur | M |
| S4-16 | E0 → Panel ürün analitiği: `POST /api/v1/panel/events` ([07](07-veri-modeli-ve-api.md) §6.3; [04](04-isletme-paneli.md) §14.4) (FE; bağımlılık S2-15) | `/store/events` ile aynı gövde ve doğrulama; `role` ve `app_version` oturumdan eklenir, kullanıcı kimliği yazılmaz. [04](04-isletme-paneli.md) §14.4 olay seti (ör. `panel_update_applied`) `events.ts` sözlüğünde. Sağlık skorunun "panel kullanımı" bileşeni ve sürüm KRI'si ([10](10-riskler-operasyon-ve-metrikler.md) §5.6, R05) bu toplamlardan beslenir | S |
...
| S5-16 | E0 → "Açık sipariş varken güncelleme" e2e testi ve güncelleme kabul testleri ([06](06-teknik-mimari.md) §16.7 madde 7; bağımlılık S3-16, S4-15) (TL, FE) | Playwright (bayraksız ses jestiyle): açık `new` sipariş varken staging'e yeni panel sürümü çıkılır; panel yeniden yüklenmez, alarm döngüsü sürer, `audio_unlocked = true` korunur; sipariş onaylanıp yoğun saat dışına çıkılınca (sahte saat) güncelleme uygulanır ve P-03 açılır. Ses kilidi kurtarma zinciri (bant, Web Push, 5 dk `owner` uyarısı; sesi açık ikinci cihaz varsa uyarı yok), N−1 sözleşme testi ve "kaydedilmemiş form açıkken güncelleme yok" senaryoları yeşil. §6.3 "Panel güncelleme e2e" kapısına bağlanır; P0 maddesidir (§11.1) | M (P0 maddesi) |
...
| S6-21 | E11 → A-14 "Sürümler" ekranı **[Faz 1]** ([05](05-admin-paneli-ve-pazarlama-sitesi.md) A-14; [06](06-teknik-mimari.md) §16.7) (FE; bağımlılık S3-16) | `app_releases` kaydı ekrandan yayımlanır (yalnız PO/PA): sürüm, kanal (`pilot`/`general`), asgari sürüm ve geçiş süresi (`enforce_after`), hotfix işareti, "Yenilikler" metni. Sürüm başına cihaz dağılımı (nabızdaki `app_version`, `panel_devices_by_version`) görünür; asgari sürümü yükseltmeden önce eski sürümde kalan cihaz sayısı gösterilir. Yayımlanan satır değişmez, geri çekme yeni satırla yapılır. "Yenilikler" listesi panelde Yardım (P-39) altında görünür | S |
...
| Panel güncelleme e2e ("açık sipariş varken güncelleme", S5-16) | Panel, service worker, ses veya SSE koduna dokunan PR + her sürüm treni öncesi | Playwright, bayraksız ses jestiyle, sahte saat ([06](06-teknik-mimari.md) §16.7 madde 7) | Açık `new` siparişte yeniden yükleme yok, alarm sürer, `audio_unlocked` korunur; güvenli anda güncelleme uygulanır ve P-03 açılır | Evet; P0 kapısı |
...
|---|---|---|---|---|---|
| Su bayi | **[Faz 2]** | Damacana depozitosu (ilk siparişte depozito, boş kap iadesi); tekrarlayan sipariş ("her 10 günde 2 damacana") | Tüp/LPG satanlar hedef dışıdır; menüde tüp/LPG ürünü kategori filtresine takılır | `recurring_orders` (Faz 2; her çalıştırma `awaiting_customer` sipariş açar, müşteri onaylayınca `new` olur). Ürün düzeyinde depozito tutarı (`products.deposit_kurus`) ve müşteri başına iade bekleyen kap sayısı (`customer_deposits`, `deposit_movements`) [07](07-veri-modeli-ve-api.md) §3.8'e eklendi (taslak; 07 §11 #29) | F2-16 |
| Pastane | **[Faz 2]** | Ön sipariş (teslim günü ve saati), özel pasta formu (kat, yazı, örnek görsel), ön siparişte kapora | Alkollü ürünler (ör. likörlü tatlı) ürün bayrağıyla engellenir (kapsamı teyit edilmeli). Alerji bilgisi yapılandırılmış alan olarak tutulmaz, yalnız serbest sipariş notu ([00](00-kararlar-ve-sozluk.md) §9) | Planlı sipariş `orders.scheduled_for` (Faz 2). Kapora online ödemeyle (F2-11, işletmenin kendi PSP hesabı). Özel pasta formunun yapılandırılmış alanları ve görsel eki [07](07-veri-modeli-ve-api.md) §3.8'e eklendi (`products.custom_form_schema`, `order_custom_forms`, `order_attachments`; alerji alanı yok); görselin medya saklama süresi [08](08-mevzuat-kvkk-odeme-fatura.md) §2.8'e bağlanır (teyit edilmeli) | F2-16 |
| Market / şarküteri | **[Faz 3]** | Geniş katalog (Excel içe aktarma, P-14), tartılı ürün, stok | Alkol ve tütün ürün bayrağıyla engellenir ([00](00-kararlar-ve-sozluk.md) §9); tekel hedef dışıdır | Tartılı ürün birimi (`products.unit`, `weight_step`) ve stok tabloları [07](07-veri-modeli-ve-api.md) §3.8'de Faz 3 taslağı olarak tanımlı; ondalık miktar ve tartı farkı kuralı Faz 3 tasarımında | F3-11 |
...
- [ ] Panel güncelleme politikası çalışıyor ([06](06-teknik-mimari.md) §16.7; S3-16, S4-15): `app_releases` kaydı, service worker "prompt" modu, güvenli an / "Vardiya başlat" kuralı, asgari sürüm başlıkları; "açık sipariş varken güncelleme" e2e testi (S5-16) ve ses kilidi kurtarma zinciri yeşil; sürüm treni ve hotfix yolu staging'de bir kez işletildi.
...
31. **Son temizlik turu eklemeleri:** [06](06-teknik-mimari.md) §16.7 sürüm yönetimi (S3-16, S4-15, S5-16, S6-21; e2e testi §6.3'te kapı, §11.1'de P0 maddesi), ürün analitiği (S2-15, S4-16; [07](07-veri-modeli-ve-api.md) §3.5, §6.2–6.3) ve Faz 1 i18n altyapısı (S1-12) sprintlere eklendi; kapasite etkisi §5.2 "Platform işleri eki"ndedir [T]. Dikey uyarlamaların veri modeli karşılıkları (depozito, özel pasta formu, tartılı ürün ve stok) [07](07-veri-modeli-ve-api.md) §3.8'e taslak olarak girdi (§8.2.1; 07 §11 #29). Açık: service worker "prompt" modu ve iOS ana ekran PWA güncelleme davranışı ([06](06-teknik-mimari.md) §16.7, teyit edilmeli); analitik oturum kimliğinin rıza gereksinimi ([07](07-veri-modeli-ve-api.md) §11 #25).
