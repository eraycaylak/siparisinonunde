...
| # | Konu | Durum / öneri |
|---|---|---|
| 1 | **Webhook alımının iki ayrı sunucu/VM'de olması** (eski 06 pilot topolojisinde `api-hooks` ×2 aynı makinedeydi). | **Karara bağlandı:** [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §11 "en az iki ayrı sunucu/VM üzerinde webhook alımı (pilotta ucuz ikinci VPS yeterli)"; bu doküman uygular (§1, R14, RB-1), [06](06-teknik-mimari.md) §13.3 ikinci VPS + yerel kalıcı spool ile hizalandı. |
| 2 | **Sentetik canary** (pilot öncesi zorunlu paket). | **Karara bağlandı:** işaret `test_kind = 'canary'` ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5); tasarım [06](06-teknik-mimari.md) §7.10 (`canary-tenant`, `canary-platform` işleri; `canary_ack_seconds{branch}`, `canary_e2e_seconds`), veri modeli [07](07-veri-modeli-ve-api.md) §4.1 (≤ 10 dk'da silme). Açık kalan: canary numaraları arası otomatik mesajlaşmanın Meta politikasına uygunluğu ve maliyeti teyit edilmeli. |
| 3 | **"Kaçan sipariş" tanımı** ve "otomatik reddet". | **Karara bağlandı:** [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5 ve §10: "otomatik reddet" yoktur; 15 dk yanıtsız sipariş `cancelled` / `cancelled_by = system` / `tenant_no_response` olur. Kanonik tanım §7.2'dedir; [06](06-teknik-mimari.md) §7.6, §8.5 (`order-new-watch`) ve §14.5 buna göre yazıldı. |
...
| 7 | **Pilot süresi ile kanal payı ölçüm anı** (eski "60. gün" ifadesi pilot penceresine sığmıyordu). | **Karara bağlandı:** [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §12 ölçüm anını "pilotun 8. haftasında (pilot sonu)" olarak tanımladı; R01, D6, §8.3 ve §8.7 P2 buna göre düzeltildi, K4 Hafta 18–19'da kalır. [09](09-yol-haritasi-ve-sprint-plani.md) §7.6–7.7'deki "60. gün" ifadeleri de düzeltilmeli. |
