...
- **[Faz 1 uygulaması — pilot sürümü]** Cihaz nabzı ve `devices` tablosu henüz yok; varlık, sahip/yönetici/kasiyer oturumunun sipariş akışı (SSE) açıkken dakikada bir yazılan `branch_panel_presence` satırıdır (mutfak ve destek görünümü sayılmaz, ses kilidi sunucuya bildirilmez). Kural: şube sipariş alırken (çalışma saati içinde, duraklatılmamış, sipariş alma açık, web canlı) ekran **5 dk** görülmüyorsa ya da açılıştan beri hiç görülmeyip açılış **10 dk**'yı geçtiyse `owner`'a yalnız platform WhatsApp `isletme_panel_cevrimdisi_v1` gider, şube başına **60 dk**'da en çok 1 (`cron.panel_presence`). SMS, storefront'u otomatik durdurma ve "ses 5 dk kilitli" uyarısı sonraki sürümdedir ([04](04-isletme-paneli.md) §4.17, [15](15-kurulum-ve-isletim.md) §10).
...
| `wa-outbound` | `send` (outbox kaydı → Graph API), `onboarding_continue` (Embedded Signup sonrası `subscribed_apps`, register, şablonlar, sağlık kontrolü; `jobId = wa-onb:{waba_id}`, idempotent, [02](02-whatsapp-entegrasyonu.md) §3.5) | Numara başı token bucket (80/sn; Coexistence 20/sn) + alıcı başı ~6 sn ([02](02-whatsapp-entegrasyonu.md) §7.6) | 6, `min(1s×2ⁿ, 60s)` + jitter |
...
flowchart LR
  ...
  SUM -->|Onayla| NEW["awaiting_customer → new<br/>channel = wa_ai<br/>verification_method = wa_button"]
...
| Kaçan sipariş — **sistem kaynaklı** | 0 | Kanonik tanım [10](10-riskler-operasyon-ve-metrikler.md) §7.2 (birebir): Sipariş oluştu ama (a) `new` olduktan sonra 60 sn içinde şubenin hiçbir çevrimiçi panel cihazına (SSE / emniyet sorgusu) ulaşmadı **ve** panel dışı basamaklar (t = 0 Web Push, t = 2 dk platform WhatsApp, t = 5 dk SMS) da teslim edilemedi, **veya** (b) alarm zinciri (§7.6) planlandığı gibi çalışmadı (basamak atlandı ya da > 60 sn gecikti; 15 dk otomatik iptal çalışmadı), **veya** (c) doğrulanmış sipariş `awaiting_customer` → `new` geçişini sistem hatası yüzünden yapamadı. Kaynak: `orders`, `alarm_escalations`, `order_events` |
...
| Takip linki `/t/{token}` | `base62(HMAC-SHA256(tracking_key[kid], order_id))[:22]` (~128 bit), DB'de yalnız SHA-256 hash'i (`orders.tracking_token_hash`) | **Teslimden 7 gün sonra geçersiz** (`orders.tracking_expires_at` = teslim/final durum + 7 gün; sonrasında 410 ve yalnız kişisel veri içermeyen özet; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7, [07](07-veri-modeli-ve-api.md) §1.2) | Sayfa PII'yi maskeli gösterir (ad baş harfi, mahalle düzeyi adres), `noindex` |
...
| 6 | **02 ile tutarsızlık/eksik:** [02](02-whatsapp-entegrasyonu.md) §7.1 diyagramında panel bağlantısı "WebSocket"ti; kurye magic link'i (§6.4) ve panel çevrimdışı uyarısı (§7.7) için platform WABA şablonu 02 §5.3 listesinde yoktu. | **Karara bağlandı:** 00 §5 (SSE); 02 §7 SSE + REST'e çekildi, 02 §5.3'e `kurye_giris_v1` ve `isletme_panel_cevrimdisi_v1` eklendi. `kurye_giris_v1`'in kategorisi (utility / authentication) teyit edilmeli (02 açık konu 16). |
...
| 24 | **Entegrasyon kuyruğu** (§4.5). | **Karara bağlandı:** `integrations` kuyruğu Faz 2 için kanonik listeye eklendi ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5). Entegrasyon kodu Faz 2'de (F2-05) yazılır. |
