...
| 4 | **Kritik yol (Faz 0, hemen):** şirket → Business Portfolio + Business Verification → Meta App + Tech Provider → ES v4 yapılandırması → App Review (`whatsapp_business_messaging`, `whatsapp_business_management`, video) → onboarding limiti kayan 7 günde 10 → doğrulama sonrası kayan 7 günde 200 ([00](00-kararlar-ve-sozluk.md) §6.3; pencerenin kayan mı takvim haftası mı olduğu [13](13-varsayim-ve-teyit-kaydi.md) V-019). ES v2 8 Ekim 2026'da kalkıyor; yalnız v4 geliştirilir. | Faz 0 |
...
| 7 | **Maliyet defteri ve pass-through:** her status webhook'undaki `pricing` nesnesi kaydedilir; panelde "bu ay Meta'ya tahmini ödeme". Aboneliğe "WhatsApp mesajları dahil" vaadi verilmez (Faz 3 MPS'e kadar). Kampanya modülünde (Faz 2) gönderim öncesi maliyet önizlemesi ve İYS sorgusu zorunlu; durum şablonlarına promosyon/indirim kodu konamaz. | Faz 1 / Faz 2 |
...
| 11 | **Kademeli yeni sipariş alarmı (kanonik zamanlama):** t=0 panel sesi + Web Push → t=60 sn ses tekrarı (yükselen) → t=2 dk ayrı **platform WhatsApp numarasından** işletme sahibine uyarı şablonu → t=5 dk SMS → t=10 dk müşteriye "henüz onaylanmadı" bilgisi → t=15 dk otomatik `cancelled` (`tenant_no_response`) + müşteriye özür ve işletme telefonu (§10.3). Otomatik iptal süresi işletme ayarıyla 10–30 dk; müşteri bilgisi iptalden en az 5 dk önce. "Otomatik reddet" yoktur. | Faz 1 |
...
flowchart LR
  ...
  B --> D[Meta App + webhook<br/>+ Tech Provider] --> E[ES v4 yapılandırması] --> F[Demo ortamı + videolar] --> G[App Review] --> H[Live mod<br/>onboarding: kayan 7 günde 10]
  C & G --> I[Onboarding: kayan 7 günde 200]
...
| ☐ | Ayrı **platform WhatsApp numarası** (kendi portföyümüzde WABA, Cloud API; alarm, panel çevrimdışı, kurye giriş linki, olay duyurusu ve abonelik şablonları, §5.3, §10.3; aynı numara işletmelerin **destek WhatsApp hattıdır**, [00](00-kararlar-ve-sozluk.md) §4) | Teknik lider | 1 gün | Numara kayıtlı; §5.3'teki Faz 1 şablonları pilot öncesi `APPROVED` |
...
- `sms_fallback` kill-switch'i kapatıldığında OTP ve kritik durum SMS'leri gönderilmez; WhatsApp'ı bağlı işletmede Akış B yalnız WhatsApp ile çalışır ("SMS ile doğrula" gizlenir), bağlı olmayan işletmede storefront telefonla sipariş yönlendirmesini gösterir.
...
Hedef: sipariş kaçırma %0. Zamanlama kanoniktir ([00](00-kararlar-ve-sozluk.md) §10) ve aşağıdaki tabloyla birebir aynıdır; süreler işletme ayarıyla, platformun min/maks sınırları içinde değiştirilebilir (`branches.alarm_policy`, `branches.new_order_timeout_min`): **otomatik iptal süresi 10–30 dk** (varsayılan 15), **müşteriye bilgi otomatik iptalden en az 5 dk önce** gider (varsayılan 10. dk; iptal 10 dk'ya çekilirse bilgi en geç 5. dk'da). t=0, 60 sn, 2 dk ve 5 dk basamaklarının sırası değişmez. Zincir, sipariş `new` olduğu anda (Akış A, E) veya `awaiting_customer → new` geçişinde (Akış B/C doğrulaması) başlar; `awaiting_customer` siparişte ses ve alarm yoktur. Her basamak `notify` kuyruğunda `alarm:{order_id}:{step}` anahtarıyla idempotenttir ve çalışmadan önce siparişin hâlâ `new` olduğunu ve **bekleyen ret** (`rejection_scheduled_at` dolu) olmadığını kontrol eder. Zincir sipariş `new`'den çıktığında (`accepted`, `rejected`, `cancelled`) durur; bekleyen ret sırasında duraklar, 30 sn içinde "Geri al" basılırsa kalan basamaklar kaldığı yerden sürer. Otomatik kabul Faz 2'de, kurallı ve varsayılan kapalıdır. Panel tarafı (ses kilidi, Web Push, çevrimdışı dedektörü) [04](04-isletme-paneli.md)/[06](06-teknik-mimari.md)'da.
...
| 5 | t+10 dk (otomatik iptalden en az 5 dk önce) | Müşteriye "işletme henüz onaylamadı" bilgisi: pencere içi serbest mesaj, butonlar [Beklerim] [Siparişi iptal et] ([03](03-musteri-deneyimi-ve-storefront.md) M13) | Müşteri | Sipariş başına ≤ 1, bütçe dışı; pencere kapalıysa gönderilmez. WhatsApp'sız modda yalnız takip sayfasında gösterilir. [Siparişi iptal et] → `new → cancelled` (`cancelled_by = customer`, `customer_request`) |
| 6 | t+15 dk (ayar: 10–30 dk) | Otomatik `new → cancelled` (`cancelled_by = system`, `cancel_reason = tenant_no_response`) | Müşteri, işletme | Müşteriye özür + işletme telefonu: pencere içinde M12d, dışında `siparis_iptal_yanitsiz_v1` (§5.2), WhatsApp'sız modda SMS-03b; bütçe dışı. İşletmeye panel bildirimi ve e-posta. **"Otomatik reddet" yoktur**; sistem siparişi `rejected` yapmaz |
...
- **Onboarding test siparişi** (`test_kind = 'onboarding_test'`, [00](00-kararlar-ve-sozluk.md) §7): kısaltılmış zincir — yalnız basamak 1–3 kurulur; basamak 3 "TEST #1001" biçimli sipariş no parametresiyle gider (§5.3 notları; [06](06-teknik-mimari.md) §7.6, [07](07-veri-modeli-ve-api.md) §4.1). Basamak 4–6 atlanır, otomatik iptal yoktur; işletme test siparişini onaylayarak veya reddederek kapatır.
