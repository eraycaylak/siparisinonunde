...
- **Tablolar çoğul `snake_case`** (`tenants`, `orders`, `order_items`, `branch_events`; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5 "Adlandırma"). Sözlükteki tekil adlar varlık adıdır. `audit_log` ve `tenant_usage_daily` kütle adı olarak istisnadır. Kolonlar `snake_case`, Drizzle nesneleri camelCase, API JSON alanları `snake_case`.
- Enum değerleri [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) ile **birebir**. Kararlı listeler `pgEnum`: `order_status`, `payment_status`, `payment_method`, `fulfillment_type`, `order_channel`, `tenant_role`, `platform_role`, `reseller_role`, `subscription_status`, `wa_mode`, `conversation_state`, `conversation_mode`. Değişebilecek listeler (sebep kodları, `verification_method`, `test_kind`, bildirim türleri, `lifecycle_stage`, `onboarding_step`, `suspension_reason`, mesaj niyeti) `text` + `CHECK`. Tek kaynak `packages/core/enums.ts` (Zod); CHECK kısıtları buradan üretilir.
...
- Dışa açık kimlikler: `orders.number` (şube bazında artan, 1001'den başlar, ekranda `#1047`); **takip token'ı** = `base62(HMAC-SHA256(tracking_key[kid], order_id))[:22]`. Token her an yeniden üretilebilir (sonraki durum mesajlarındaki link için); DB'de yalnız `tracking_token_hash` tutulur (D06 §15.3). **Takip linki teslimden 7 gün sonra geçersizdir** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7): `orders.tracking_expires_at` = `delivered_at` (ret/iptalde final an) + 7 gün; sonrasında `/t/{token}` 410 `tracking_link_expired` döner. Sipariş kodu `order_verification_codes.code` (6 karakter, D02 §6.4). Storefront'ta UUID görünmez.
...
erDiagram
  ...
  tenants ||--o{ analytics_events : "PII yok, 90 gün"
  delivery_zones {
    geography area
    int fee_kurus
...
erDiagram
  ...
  products ||--o{ product_translations : "Faz 3"
  customers ||--o{ customer_deposits : "Faz 2 su bayi"
  orders ||--o{ order_custom_forms : "Faz 2 pastane"
```
...
| Ad | Değerler | Kaynak |
|---|---|---|
| `order_status` | `awaiting_customer`, `new`, `accepted`, `preparing`, `ready`, `on_the_way`, `delivered`, `rejected`, `cancelled` | [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5 |
...
| `wa_mode` | `cloud`, `coexistence` | [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §3 |
...
| `brand_color` | text | ✓ | **[Faz 1]** İşletmenin ana rengi, `#RRGGBB` (CHECK `~ '^#[0-9A-Fa-f]{6}$'`). P-26 ve onboarding 2. adımında seçilir ([04](04-isletme-paneli.md) §7.2, §3.3). Boşsa hazır paletin ilk rengi kullanılır ([12](12-marka-tasarim-ve-kullanilabilirlik.md) §5.1). Durum rozetinde, hata ve uyarıda kullanılmaz |
| `brand_palette` | jsonb | ✓ | `brand_color`'dan sunucuda türetilen tema değişkenleri (`--brand`, `--brand-contrast`, `--brand-strong`, `--brand-ui`, `--brand-subtle`, koyu tema karşılıkları) ve "koyulaştırıldı" bayrağı. Kayıtta `packages/ui/theme/brandPalette()` ile hesaplanır, SSR'da satır içi yazılır ([12](12-marka-tasarim-ve-kullanilabilirlik.md) §5.1). Elle yazılmaz |
| `logo_url`, `cover_url` | text | ✓ | Logo (1:1) ve kapak görseli (1,91:1) adresleri. Dosyalar R2'dedir (kişisel veri değil); `images` kuyruğu EXIF'i temizler ve varyant üretir. Boyut ve biçim kuralları [12](12-marka-tasarim-ve-kullanilabilirlik.md) §5.2. Kapak `og:image` olarak da kullanılır |
| `vertical` | text | NN | `restaurant` (Faz 1); `water`, `patisserie` (Faz 2) |
| `lifecycle_stage`, `lifecycle_changed_at` | text, timestamptz | NN | Admin yaşam döngüsü ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7, D05 §A.2.1). **Elle yazılmaz**, `core.deriveLifecycleStage()` türetir (§4.3); her değişiklik `tenant_lifecycle_events`'e yazılır. `lead` aşaması `leads` tablosunda yaşar (tenant yok) |
...
| `ordering_enabled`, `ordering_disabled_reason` | bool, text | NN / ✓ | Tenant bazında kill-switch ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4); false → storefront ve bot "şu an online sipariş alınmıyor, lütfen arayın"; varsayılan true |
...
| `alarm_policy` | jsonb | NN | Kanonik zincir ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §10) birebir varsayılan: `{"push_sec":0,"sound_repeat_sec":60,"platform_wa_sec":120,"sms_sec":300,"customer_notice_sec":600,"auto_cancel_sec":900,"platform_wa_enabled":true,"sms_enabled":true,"notify_manager":false}` — t=0 ses + Web Push, 60 sn ses tekrarı, 120 sn platform WABA uyarısı (**tek sefer**), 300 sn **yalnız SMS** (platform WhatsApp tekrarlanmaz), 600 sn müşteriye bilgi, 900 sn otomatik iptal. `auto_cancel_sec` = `new_order_timeout_min × 60` (tek kaynak kolondur; yazmada birlikte güncellenir). Basamak sırası değişmez. **Uygulama katmanı doğrular** (Zod, `packages/core`): `customer_notice_sec ≤ auto_cancel_sec − 300` (müşteri bilgisi otomatik iptalden en az 5 dk önce). "Otomatik reddet" yoktur |
| `new_order_timeout_min` | smallint | NN | `new` → `cancelled` (`cancelled_by = system`, `tenant_no_response`) süresi; varsayılan 15, **`CHECK (new_order_timeout_min BETWEEN 10 AND 30)`** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §10); planlı siparişe uygulanmaz |
...
| `brand_color`, `brand_palette`, `logo_url`, `cover_url` | text, jsonb, text, text | ✓ | Şube bazında marka geçersiz kılması ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7 "Storefront imzası": ana renk, logo ve kapak şube/işletme ayarındadır). `NULL` = `tenants`'taki değer kullanılır. Tek şubeli işletmede panelde gösterilmez; şube bazında düzenleme arayüzü çoklu şubeyle **[Faz 2]** gelir. Kurallar `tenants` satırındakiyle aynıdır |
...
- `sessions` (`token` hash, `expires_at` — işletme paneli kişisel kullanıcı oturumu **30 gün** (kayıtlı cihaz), kurye **12 saat** (vardiya) ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4); paylaşımlı cihaz kaydı `devices`'tadır (90 gün + PIN); `kind` (`panel`, `courier`, `impersonation`), `ip_address`, `user_agent` `pii:network`, `active_tenant_id`, `impersonated_by`, `impersonation_session_id ✓`), `accounts` (parola hash'i), `verifications` (kurye magic link tek kullanım, açılmazsa 15 dk'da düşer; OTP), `two_factors` (TOTP sırrı envelope encryption, yedek kodlar hash'li), `invitations` (`tenant_id`, `email`/`phone`, `role`, `branch_id`, `status`, `expires_at` 7 gün).
- `platform_users` (`adminAuth`, ayrı örnek ve çerez; D06 §6.1): `email UK NN`, `name`, `platform_role NN`, `two_factor_enabled` (her zaman true), `last_login_at`, `disabled_at`; oturumlar `platform_sessions` (**8 saat** + 30 dk hareketsizlikte kilit, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4), `platform_accounts`, `platform_two_factors`.
...
| `token_hash`, `paired_by_user_id`, `paired_at`, `expires_at` | text, uuid, timestamptz, timestamptz | ✓ | 8 haneli kodla eşleştirme; paylaşımlı cihaz **kaydı 90 gün** (`expires_at` = son kullanım + 90 gün; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4, D06 §6.3) |
| `pin_user_id`, `pin_session_started_at`, `pin_session_expires_at` | uuid, timestamptz, timestamptz | ✓ | Cihazda PIN'le giriş yapmış personel; aksiyonlar bu kullanıcı adına audit'lenir. PIN oturumu vardiya boyu, en çok 12 saat (D06 §6.2); sipariş listesi ve alarm PIN'siz görünür kalır |
...
- `plan_features` (paket hakları; feature flag değildir, D06 §16.6): PK `(plan_id, feature_key)`, `enabled`, `limit_value ✓`. Anahtarlar: `max_users` (Esnaf 2), `max_delivery_zones` (Esnaf 3), `courier_view` (Pro+), **`sms_monthly_quota`** (adil kullanım: Esnaf 100, Pro 300, Zincir şube başına 300 SMS/ay; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4), `ai_ordering`, `coupons`, `loyalty`, `campaigns`, `online_payment`, `pos_integration`, `multi_branch`, `advanced_reports`, `custom_domain`, `open_api`. İçerik matrisi D01 §6.3.

#### `tenant_usage_monthly` **[Faz 1]** (SMS kotası sayacı)
PK `(tenant_id, month date)`; `sms_count int NN` (kotaya sayılan: müşteriye giden `otp` + `order_status` SMS'leri), `sms_ops_count` (sayılmayan platform operasyon SMS'leri: `alarm`, `panel_offline`, `courier_login`), `sms_quota` (ay başında `plan_features.sms_monthly_quota`'dan kopyalanır; Zincir'de × şube sayısı), `sms_quota_warned_at ✓` (%80), `sms_quota_exceeded_at ✓`, `platform_wa_count`, `llm_tokens_in/out` (Faz 2), `updated_at`. Sayaç `sms.send` işinde aynı transaction'da `UPDATE … SET sms_count = sms_count + 1` ile artar (segment sayısı maliyet için `sms_messages.segments`'te). **Kota aşımında SMS kesilmez**: işletmeye panel + e-posta uyarısı gider, admin A-07'de görünür; ek SMS paketi Faz 2 ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4). Pilot ve `is_demo` tenant'larda kota uygulanmaz, sayım sürer.
...
| `discount_bp`, `discount_until` | int, date | ✓ | Kurucu üye: 12 ay sabit **%30 oran** (3000); liste fiyatı TÜFE ile değişebilir ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §8) |
...
| `trial_warning_started_at` | timestamptz | ✓ | Deneme bitti, 3 günlük uyarı bandı başladı ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §9) |
...
#### `customers` **[Faz 1]**
Kimlik `(tenant_id, wa_bsuid)`, telefon nullable ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §6.8, D02 §8). Platform geneli müşteri profili yoktur.
...
| `marketing_opt_in`, `marketing_opt_in_at`, `marketing_suppressed_until` | bool, timestamptz, timestamptz | | **[Faz 2]** Toplama Faz 1'de yok ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §11); kanıt `consents`'te |
...
| `number` | int | NN | `UNIQUE(branch_id, number) WHERE test_kind IS DISTINCT FROM 'canary'` (canary `0` taşır, sayacı tüketmez) |
...
| `acquisition_source` | text | ✓ | Pazaryeri sipariş beyanı ve edinim kaynağı (§3.0): `source_meta.src`'den türetilir (paket kartı QR'ı `src = Q1` → `marketplace_card`, magnet/afiş `Q2`/`Q3` → `in_store_qr`; D03 §2), müşterinin isteğe bağlı cevabı veya personelin "Bu müşteri pazaryerinden geldi" işaretiyle (`marketplace_declared`) güncellenir; değişiklik `order_events`'e yazılır |
...
| `out_of_zone_override`, `out_of_zone_override_by`, `out_of_zone_fee_kurus` | bool, uuid, int | NN / ✓ / ✓ | Yalnız `manual` kanalında: personel uyarıyı görerek bölge dışına sipariş girer ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4), özel ücret yazılabilir; `audit_log` + `order_events` |
...
| `rejection_scheduled_at`, `rejection_requested_by` | timestamptz, uuid | ✓ | **Bekleyen ret** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7): doluysa sipariş `new` kalır ve bu anda (istek + 30 sn) `rejected` olur; "Geri al" alanları temizler. D04 §4.7'deki `pending_action*` önerisinin karşılığıdır. CHECK `rejection_scheduled_at IS NULL OR status = 'new'` |
...
#### `sms_messages` **[Faz 1]**
Tüm SMS gönderimlerinin sağlayıcı kaydı: std, `branch_id ✓`, `order_id ✓`, `purpose` (`otp`, `order_status`, `alarm`, `courier_login`, `panel_offline`), `to_phone_e164` (`pii:contact`), `template_key`, `provider`, `provider_message_id ✓`, `sender_header` (Faz 1'de platformun onaylı alfanümerik başlığı, ≤ 11 karakter; mesaj gövdesinde işletme adı; işletmeye özel başlık Faz 3, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7), `status` (`queued`, `sent`, `delivered`, `failed`), `error ✓`, `segments`, `est_cost_kurus`, `counts_toward_quota bool` (`otp` ve `order_status` true; `tenant_usage_monthly`), `sent_at`, `delivered_at`. SMS maliyeti platformundur ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4). Teslim raporu webhook'la gelir (§6.6). Saklama 90 gün, sonra telefon maskelenir.

#### `storefront_link_tokens` **[Faz 1]** (Akış A)
"Menüyü aç" token'ı durumsuz HMAC'tir (D02 §6.3); tablo oturuma çevirme, iptal ve huni analizi içindir. Token gövdesine `j` (jti) eklenir = bu tablonun `id`'si. **GET'te tüketilmez:** storefront ilk açılışta `POST /store/link-session` ile HttpOnly çereze çevirir ve URL'yi temizler ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7). Alanlar: std, `branch_id`, `customer_id NN`, `conversation_id NN`, `issued_wamid ✓`, `expires_at NN` (2 sa), `first_opened_at ✓` (ilk açılış = oturuma çevrilme; D10 §8.4 huni metriği), `open_count`, `disowned_at ✓` ("Ben değilim" → sipariş Akış B'ye düşer), `order_id ✓`, `revoked_at ✓`, `prefill_cart jsonb ✓` (Faz 2 [Düzenle]). Saklama 30 gün.

#### `couriers`, `courier_assignments` **[Faz 1]**
- `couriers`: kurye bir `user` + `memberships.role = 'courier'`dır ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §3); bu tablo yalnız kuryeye özgü profil verisidir. std, `membership_id UK`, `user_id`, `branch_id`, `display_name`, `phone_e164` (`pii:contact`), `vehicle`, `is_on_shift`, `shift_started_at`, `last_seen_at`, `deleted_at`. Pro ve Zincir'de (`courier_view`).
...
Token kolonları yalnız `wa-outbound` ve onboarding kodunun ayrı Drizzle projeksiyonunda okunur. **Platform WABA'sı** (işletmeye uyarı şablonlarını gönderen ve aynı zamanda işletmelerin destek WhatsApp hattı olan numara, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4 "Destek hattı ve P1") kendi iç **`platform` tenant'ında** modellenir (`tenants` satırı; `wa_accounts`, `wa_phone_numbers`, `conversations`, `messages` aynı tablolar ve aynı konuşma motoru). İşletme sahiplerinin yazdığı mesajlar ve uyarı şablonlarına verdikleri yanıtlar bu tenant'ın sohbetleri olarak admin panelindeki destek gelen kutusuna düşer (yalnız platform rolleri erişir); uyarı gönderimleri ayrıca hedef işletmenin `notifications` kaydına loglanır. `platform` tenant'ı metriklerden, MRR'dan ve faturalamadan hariçtir (`is_demo = true` gibi işlenir).
...
| `last_welcome_at`, `last_nudge_at` | timestamptz | ✓ | Karşılama sıklığı ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7): tam karşılama (menü linkli) en fazla 12 saatte bir; arada kısa yanıt + "Menüyü aç" en fazla 30 dk'da bir; açık siparişi olana karşılama yerine sipariş durumu kartı |
...
#### `analytics_events` **[Faz 1]** (ürün analitiği olayları)
Storefront hunisi ([03](03-musteri-deneyimi-ve-storefront.md) §11) ve panel kullanım olayları ([04](04-isletme-paneli.md) §14.4) için birinci taraf, çerezsiz olay deposu. Domain olaylarından (§7.1) ayrıdır; iş kuralı bu tabloyu okumaz.

| Alan | Tip | Null | Açıklama |
|---|---|---|---|
| `id` | uuid | NN | UUIDv7; PK `(id, received_at)` (partition anahtarı) |
| `tenant_id`, `branch_id` | uuid | NN / ✓ | Storefront'ta `Host`'tan, panelde oturumdan çözülür; istek gövdesindeki kiracı alanı yok sayılır |
| `name` | text | NN | Olay adı. **Tek olay sözlüğü `packages/core/events.ts`**'tedir: her olayın adı, kaynağı, fazı ve Zod şeması orada tanımlıdır; sözlükte olmayan olay reddedilir |
| `source` | text | NN | `storefront`, `panel`, `server` (domain olayından türetilen huni adımı, ör. `order_created`) |
| `occurred_at`, `received_at` | timestamptz | NN | İstemci saati (± 24 sa dışındaysa `received_at`'e eşitlenir) / sunucu alım anı |
| `session_id` | text | ✓ | Sekme oturumu boyunca geçerli rastgele değer (`sessionStorage`, [03](03-musteri-deneyimi-ve-storefront.md) §11). Kalıcı cihaz veya kişi kimliği değildir; müşteri, kullanıcı ve sipariş kimliğiyle **birleştirilmez** |
| `src`, `channel_ctx`, `returning` | text, text, bool | ✓ | Giriş kaynağı (`qr`, `paket`, `ig` …), `wa`/`web`, "Son siparişin" kartı görüldü mü |
| `props` | jsonb | NN | Olaya özgü alanlar; şema sözlükten gelir, izin listesi dışı anahtar atılır. Serbest metin alanı yoktur |
| `device_class`, `in_app_browser`, `app_version` | text | ✓ | `mobile`/`tablet`/`desktop`; `wa`/`ig`/`google`/`none`; panel sürümü. Ham user agent ve IP **saklanmaz** |
| `role` | text | ✓ | Yalnız panel olaylarında rol (`owner`, `cashier` …); kullanıcı kimliği yazılmaz |

- **PII yok:** Ad, telefon, adres, not, e-posta, BSUID, IP, kullanıcı ve müşteri kimliği bu tabloya girmez. Zod şemaları yalnız izin listesindeki alanları kabul eder, CI şema lint'i `pii:` işaretli kolon adlarının `props` şemasında geçmediğini doğrular (§10).
- **Bölümleme ve saklama:** Aylık partition (`received_at`). **90 gün** saklanır, süresi dolan partition düşürülür (`retention.analytics`, §9; [08](08-mevzuat-kvkk-odeme-fatura.md) §2.8 satır 20). Raporlanacak huni özetleri gerekiyorsa kişisel veri içermeyen günlük toplamlar ayrıca tutulur [T].
- **RLS:** Standart `tenant_isolation` + `platform_read`. İşletme paneli bu tabloyu doğrudan göstermez; admin hunileri (D05 A-05, A-19) ve ürün kararları için okunur. Yazma yalnız §6.2 `POST /store/events` ve §6.3 `POST /events` uçlarından ve domain olayı türeten worker'dan yapılır.
- **Test siparişleri:** `test_kind` dolu siparişlerden türeyen sunucu olayları yazılmaz; `is_demo` tenant'ların olayları metriklere girmez.
- İndeks `(tenant_id, name, received_at)`. Olay hacmi pilot verisiyle ölçülür; `received_at` için BRIN indeks değerlendirilir.
...
| `app_releases` | 1 | Uygulama sürüm kaydı ve panel güncelleme politikası (D06 §16.7): `app` (`panel`, `admin`, `web`; Faz 2 `android_app`, `print_agent`), `version` (semver), `channel` (`pilot` = öncü halka 0–1, `general`), `released_at`, `min_supported_version`, `enforce_after` (geçiş süresi sonu), `is_hotfix`, `notes_md` ("Yenilikler" metni, esnaf dilinde; D04 §14.3), `released_by_platform_user_id`. `UNIQUE(app, version)`; yayımlanan satır değişmez, geri çekme yeni satırla yapılır. Nabız yanıtı (§6.3 `POST /devices/heartbeat`) tenant'ın halkasına göre hedef sürümü ve asgari sürümü buradan döner |
...
| `tenant_health_scores` | 1 | Günlük sağlık skoru (D10 §5.6; admin listesi D05 A-03): PK `(tenant_id, score_date)`, `score smallint` (0–100), `band` (`green` ≥ 75, `yellow` 50–74, `red` < 50), `components jsonb` (kanal siparişi eğilimi 30, panel kullanımı 20 — kaynak `analytics_events` günlük toplamları + `devices` nabzı, operasyon kalitesi 15, WhatsApp sağlığı 15, destek sinyali 10, ticari durum 10), `red_triggers text[]` (7 günde < 2 kanal siparişi, SEV1/SEV2 etkisi, 3 gün panel açılmaması, `past_due`, WhatsApp kırmızı), `computed_at`. 06:00'a kadar `cron` ile; son değer `tenants.health_*`'a kopyalanır; kırmızıya düşüşte `admin_tasks` açılır. Saklama 2 yıl |
...
| `customer_deposits`, `deposit_movements` (+ `products.deposit_kurus`) | 2 | **Su bayi damacana depozitosu** ([09](09-yol-haritasi-ve-sprint-plani.md) §8.2.1, F2-16). Ürün düzeyi: `products.deposit_kurus ✓` (kap başına depozito; null = depozitosuz ürün). Müşteri bakiyesi: `customer_deposits(tenant_id, customer_id, product_id, containers_out smallint NN ≥ 0, deposit_balance_kurus NN, updated_at)`, `UNIQUE(tenant_id, customer_id, product_id)` — müşteri başına iade bekleyen kap sayısı. Hareket (append-only): `deposit_movements(order_id ✓, customer_id, product_id, delta_containers, delta_kurus, kind: charged, returned, refunded, adjusted, recorded_by_user_id ✓)`; kurye/kasiyer teslimde "alınan boş kap" sayısını girer. Depozito siparişte ayrı kalem olarak gösterilir, fiyat sunucuda hesaplanır (§5); KDV ve muhasebe niteliği mali müşavirle teyit edilmeli [T]. Müşteri anonimleştirmesinde bakiye satırı `customer_id` ile birlikte anonimleşir [T] |
| `order_custom_forms`, `order_attachments` (+ `products.custom_form_schema`) | 2 | **Pastane özel pasta formu** ([09](09-yol-haritasi-ve-sprint-plani.md) §8.2.1, F2-16). Şablon: `products.custom_form_schema jsonb ✓` (ör. kat, kişi sayısı, pasta üstü yazı, tema; alan listesi pilot pastanelerle kesinleşir [T]); yapılandırılmış alerji/sağlık alanı **eklenemez** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §9; §10 şema lint'i form şemasına da uygulanır). Cevaplar: `order_custom_forms(tenant_id, order_id, order_item_id, schema_version, answers jsonb NN)`; `answers` serbest metin içerebildiğinden `pii:content-sensitive`, `orders.notes` ile aynı saklama kuralı [T]. Örnek görsel: `order_attachments(tenant_id, order_id, storage_key, mime_type, size_bytes, purge_at, purged_at ✓)` — müşteri medyasıdır, TR nesne deposunda tutulur (R2'de değil; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §10), boyut/tip sınırı ve saklama süresi D08 §2.8'e yeni satır olarak bağlanır (varsayılan `retention.media` ile aynı [T]). Ön sipariş `orders.scheduled_for` (Faz 2), kapora `order_payments` (F2-11) |
| `products.unit`, `products.weight_step` (+ stok tabloları) | 3 | **Market/şarküteri tartılı ürün ve stok** ([09](09-yol-haritasi-ve-sprint-plani.md) §8.2.1, F3-11). `products.unit` (`piece` varsayılan, `kg`, `g`, `l`), `weight_step ✓` (sepette artış adımı), `min_quantity ✓`; `price_kurus` birim başına yorumlanır. Tartılı üründe `order_items.quantity smallint` yetmez → ondalık miktar alanı ve "tahmini tutar / tartı farkı" kuralı Faz 3 tasarımında (mesafeli satış özetine etkisi D08 ile teyit edilmeli [T]). Stok: şube bazında `product_stock(branch_id, product_id, on_hand, reorder_level ✓)` + append-only `stock_movements` [T]; Faz 1–2'de stok yalnız `is_available` / `sold_out_until` anahtarıdır |
...
| `api_keys`, `webhook_subscriptions` | 3 | Açık API anahtar hash'i + kapsam; imzalı giden webhook. Olay adları §7.1 kataloğundandır, yük şemaları `packages/contracts`'ta sürümlüdür (D06 §4.5) |
| `product_translations` | 3 | Çok dilli storefront (EN, RU, AR, DE; turistik bölgeler, D06 §4.3): std, `product_id`, `locale` (`en`, `ru`, `ar`, `de`), `name NN`, `description ✓`, `source` (`manual`, `machine_reviewed`), `updated_by_user_id ✓`. `UNIQUE(tenant_id, product_id, locale)`; bileşik FK. Çeviri yoksa Türkçe ad gösterilir. Kategori ve seçenek adları için aynı desen Faz 3 tasarımında eklenir [T]. Fiyat, alerjen ve `wa_restricted` çeviriden bağımsızdır (tek kaynak `products`) |

**Dikey uyarlamalar ve Commerce Policy ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §6.10, §11 segment sırası; [09](09-yol-haritasi-ve-sprint-plani.md) §8.2.1):** Su bayi, pastane (Faz 2) ve market/şarküteri (Faz 3) alanları yukarıdaki üç satırdadır; alan adları taslaktır, F2-16 / F3-11 tasarımında kesinleşir (§11 #29). Yeni dikey alanları Commerce Policy engelini değiştirmez: tüp/LPG (`restricted_reason = hazardous`), likörlü tatlı gibi alkollü ürün (`alcohol`) ve marketteki alkol/tütün (`alcohol`, `tobacco`) `wa_restricted = true` ile sepete eklenemez (§3.2 `categories`/`products`). Tüp bayi, eczane, tekel ve nargile kafe tenant olarak hedeflenmez; dikey açılışında kategori filtresi ve yasaklı ürün taraması (`abuse_signals.kind = prohibited_item`, §3.7) dikeyin tipik ürün adlarıyla güncellenir [T].
...
| 4 | `awaiting_customer` → `cancelled` | System (30 dk); müşteri (takip sayfası, Akış C [İptal]) | `system` + `customer_timeout` / `customer` + `customer_request` | Kod/OTP `expired`; Akış B'de mesaj yok (pencere yok); Akış C/D [Faz 2] zaman aşımında M12c, [İptal]'de kısa teyit mesajı (D02 §5.2, §6.5; D03 §9.2) |
...
| 7 | `new` → `cancelled` | Müşteri doğrudan (takip sayfası, gecikme mesajındaki [İptal]; talep açılmaz); system (`order-new-watch`, `new_order_timeout_min` = 15; planlı ve `onboarding_test` siparişinde çalışmaz) | `customer` + `customer_request` / `system` + `tenant_no_response` | Sistem iptalinde müşteriye özür + işletme telefonu (bütçe dışı istisna); işletmeye bildirim ve ayrı ses; alarm kapanır |
...
**Kurallar:**
- **Bekleyen ret (30 sn geri alma, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7):** `POST /orders/{id}/reject` durumu değiştirmez; sipariş `new` kalır. Sebep, `rejection_scheduled_at = now + 30 sn` ve `rejection_requested_by` yazılır, alarm adımları duraklar (`alarm_escalations.cancel_reason = rejection_scheduled`) ve `available_at = rejection_scheduled_at` ile `order.finalize_rejection` outbox kaydı (iptal edilebilir gecikmeli iş) açılır. `POST …/reject/undo` 30 sn içinde bu kaydı `cancelled` yapar, alanları temizler, alarm zinciri kaldığı yerden sürer. Süre dolunca `notify` işçisi geçiş 6'yı çalıştırır, müşteri mesajı o transaction'da outbox'a yazılır. `rejected → new` geçişi yoktur. Bekleyen ret sırasında `accept` 409 `rejection_pending` döner; müşteri iptali ve 15 dk sistem iptali önceliklidir (bekleyen ret düşer).
- **Debounce (60 sn, yalnız Akış A):** "alındı" kaydı `available_at = placed_at + 60 sn` ile yazılır. Bu sürede onay gelirse kayıt `superseded` olur ve tek "alındı + onaylandı" mesajı gider (1 mesaj sayılır). Akış B'de doğrulama koduna "alındı" yanıtı anında gider; Akış E'de tek "onaylandı" mesajı gider ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7).
- **Mesaj bütçesi:** sipariş başına ≤ 4 otomatik durum mesajı (`wa_status_msg_count`; Akış A karşılama + "Menüyü aç" buna ek 1 mesajdır, toplam ≤ 5). Red, iptal ve gecikme/özür mesajları istisnadır ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §6.5). Yerine geçme ve tazelik kuralları D02 §4.3, §7.7'dedir.
- **Müşteri iptali ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7):** `awaiting_customer` ve `new`'de müşteri doğrudan iptal eder (geçiş 4, 7). `accepted` ve sonrasında yalnız **iptal talebi** açılır: `cancellation_requests` satırı + `orders.cancel_requested_at` + `order.updated` + ayrı ses. İşletme onaylarsa geçiş 12 (`cancelled_by = customer`, `customer_request`, onaylayan `audit_log`'a); reddederse talep `declined` ve `order_events.cancel_request_declined`.
...
- **Test siparişinde alarm (`onboarding_test`, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7):** Kısaltılmış zincir çalışır. `alarm_escalations`'a yalnız adım 1 (t0 ses + Web Push), 2 (60 sn ses tekrarı) ve 3 (2 dk platform WhatsApp) yazılır. Adım 3 "TEST" etiketiyle gider (sipariş no parametresi "TEST #1001") ve yalnız `owner`'ın platform WhatsApp izni (`consents.purpose = platform_wa_alerts`) varsa gönderilir. Adım 4 (SMS), 5 (müşteriye gecikme bilgisi) ve 6 (otomatik iptal) hiç planlanmaz; `order-new-watch` bu siparişi `tenant_no_response` ile iptal etmez. Sipariş `new`'de kalır, işletme onaylayarak (geçiş 5) veya reddederek (geçiş 6) kapatır; geçişlerin durum mesajları normal gider (sihirbazın amacı esnafın "Onaylandı" mesajını görmesidir, D04 §3.4.3). `canary` siparişinde alarm zinciri kurulmaz ve hiçbir dış bildirim gitmez (aşağıda).
- **Sentetik canary (pilot öncesi zorunlu paket, [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §11; D06 §7.10 ile aynı):** *Tenant canary* — `cron` kuyruğundaki `canary-tenant` işi her şubenin açık saatlerinde 15 dk'da bir [T] storefront API'sine platform imzalı `X-Canary` başlığıyla sabit sentetik sepet gönderir; sipariş `test_kind = 'canary'` ile gerçek yoldan geçer (fiyat hesabı → DB → `branch_events` (`is_canary`) → SSE). İmzasız istekte `test_kind` yok sayılır. Panel bunu göstermez ve ses çalmaz, yalnız otomatik ack gönderir. Alarm zinciri, yazdırma ve müşteri mesajı çalışmaz (WA adımı dry-run); FSM'de iptal geçişi kullanılmaz; işletmenin sipariş numarası sayacını tüketmez (`number = 0`, `UNIQUE(branch_id, number) WHERE test_kind IS DISTINCT FROM 'canary'`). Ack süresi `canary_ack_seconds{branch}` metriğine yazılır; "çevrimiçi" cihaz varken 60 sn içinde ack gelmezse SSE'ye `resync`, 2 ardışık başarısızlıkta `owner`'a `stale_panel` uyarısı (panel çevrimdışı akışı). Kayıt **ack alınınca veya en geç 10 dk sonra** `sys_purge_canary()` ile kalıcı silinir; ölçüm yalnız metriklerde kalır. *Platform canary* (Meta dahil) yalnız `sandbox` tenant'ında çalışır ve `canary_e2e_seconds`'ı üretir. Canary ve `onboarding_test` siparişleri rapor, KPI, müşteri istatistikleri, kota ve faturalamadan hariçtir.
...
- Karşılama sıklığı: `greeting` yalnız `last_welcome_at` 12 saatten eskiyse tam karşılama gönderir; arada kısa yanıt + "Menüyü aç" `last_nudge_at` 30 dk'dan eskiyse; `active_order_id` doluysa karşılama yerine sipariş durumu kartı ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7). Gelen her mesaja `messages.intent` yazılır.
...
**Deneme bitişi ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §9):** 14. gün dolunca plan seçilmediyse `trial_warning_started_at` yazılır ve 3 gün uyarı bandı gösterilir → D+3 `suspended` (`suspension_reason = trial_ended`; sipariş alma durur) → 90 gün içinde plan seçilirse veriler aynen döner → D+90 `cancelled` ve silme. Pilot bitişinde plan seçilmezse aynı kural (`pilot_ended`).
...
6. **Teslimat:** `delivery` → konum zorunlu; en yüksek öncelikli aktif `ST_Covers` bölgesi; yoksa `out_of_delivery_area` (istisna: `manual` kanalında personel uyarıyı görüp `out_of_zone_override` ile kaydedebilir; ücret `out_of_zone_fee_kurus`, min sepet uygulanmaz; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4). Min sepet indirim öncesi ara toplama uygulanır → `min_basket_not_met` (eksik tutarla). Ücret: `free_over_kurus` varsa ve ara toplam ≥ eşik ise 0, değilse `fee_kurus`. `pickup` → ücret 0, `min_order_pickup_kurus`.
...
|---|---|---|---|
| Panel oturumu (`panelAuth`) | O, M, C (+ RA, RT Faz 2) | `__Host-` HttpOnly çerez, **30 gün** (kayıtlı cihaz; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4) | `owner`, platform ve bayi kullanıcıları için TOTP zorunlu; kritik işlemlerde taze oturum (10 dk) |
| Cihaz oturumu | Paylaşılan kasa/mutfak tableti (K, C) | Cihaz kaydı çerezi (**90 gün**) + personel PIN'i (vardiya, en çok 12 saat) | Rol cihazdan gelir ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4) |
| Kurye oturumu | Ku | Tek kullanımlık magic link → **12 saatlik** vardiya çerezi ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4) | Yalnız `/courier/*` |
...
| `app_version_unsupported` | 426 [T] | Panel sürümü `app_releases.min_supported_version`'ın altında ve `enforce_after` geçmiş; **yalnız sipariş dışı uçlarda** (menü, ayar, dışa aktarma) döner, sipariş aksiyonları hiçbir sürümde engellenmez (D06 §16.7) |
...
| POST | `/store/events` | — (anonim; çerez yok) | Ürün analitiği ([03](03-musteri-deneyimi-ve-storefront.md) §11): `sendBeacon` ile `{session_id, events: [{name, occurred_at, props}]}` toplu gönderim (istek başına ≤ 20 olay [T]). Her olay `packages/core/events.ts` sözlüğündeki Zod şemasıyla doğrulanır; sözlük dışı olay ve izin listesi dışı alan atılır, PII kalıbı (telefon, e-posta) taşıyan istek bütünüyle reddedilir. Kiracı `Host`'tan çözülür. `analytics_events`'e yazılır (§3.5), yanıt `204`. IP ve user agent saklanmaz; yalnız rate limit ve `device_class`/`in_app_browser` türetimi için anlık kullanılır. Turnstile gerekmez; IP başına limit (§6.1) uygulanır |
...
| POST | `/devices/heartbeat`, `/devices/push-subscription` | tümü | `{audio_unlocked, wake_lock_active, visible, app_version, last_event_seq}`; nabız yanıtı `{target_app_version, min_supported_version, enforce_after}` döner (tenant'ın öncü halkasına göre `app_releases`'tan; D06 §16.7) |
| POST | `/events` | tümü (cihaz dahil) | Panel ürün analitiği olayları (D04 §14.4): `/store/events` ile aynı gövde ve doğrulama; `role` ve `app_version` oturumdan eklenir, kullanıcı kimliği yazılmaz; `analytics_events` (§3.5) |
...
| GET / PATCH | `/tenant`, `/branches/{b}` | O (M kısmi) | Profil, marka (`brand_color`; logo ve kapak `/uploads/images` ile yüklenir, `logo_url`/`cover_url` yazılır; `brand_palette` sunucuda türetilir), bot, mesaj ayarları, saklama süresi, tasarruf oranı; operasyon ayarları |
...
|---|---|---|
| POST | `/courier/auth/verify` | Magic link token'ı → 12 saatlik vardiya oturumu ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4); ikinci kullanım reddedilir |
...
### 7.1 Olay kataloğu
Olay adları SSE ile ortaktır (D06 §7.2). Her olay, kaynak değişiklikle aynı transaction'da `branch_events` ve/veya `outbox` satırı üretir. Entegrasyon adaptörleri (Faz 2) ve açık API giden webhook'ları (Faz 3) yeni olay adı icat etmez, bu katalogdaki adları sürümlü yük şemalarıyla kullanır (D06 §4.5). Ürün analitiği olayları (`packages/core/events.ts`, `analytics_events`, §3.5) bu katalogdan ayrıdır: domain olayı değildir ve hiçbir iş kuralını tetiklemez.

| Olay | Üretici | Tüketiciler ve yan etkiler |
|---|---|---|
| `order.created` | storefront API, panel API, wa-inbound (Faz 2 AI), `cron` (canary) | SSE + ses (awaiting ve canary hariç); alarm zinciri; "alındı" (Akış A'da 60 sn debounce); yazdırma; rapor rollup |
| `order.updated` (durum geçişleri, doğrulama, bekleyen ret/geri alma, ETA, gecikme, ödeme, kurye, iptal talebi ve kararı, düzenleme, edinim kaynağı) | panel, kurye, wa-inbound, storefront, `cron` | SSE; geçişe göre mesaj (§4.1); alarm iptali; yazdırma; `customers` istatistikleri; iade (Faz 2) |
...
| `subscription.status_changed` | faturalama cron'u, PSP webhook'u, admin | Erişim kapıları (§4.3); `lifecycle_stage` yeniden türetilir; bildirim |
| `tenant.lifecycle_changed` / `tenant.onboarding_step_completed` | `core.deriveLifecycleStage()`, onboarding servisi | `tenant_lifecycle_events`, `tenant_onboarding_steps`; admin hunisi; takılan adım görevi (`admin_tasks`) |
| `tenant.suspended` / `tenant.unsuspended` / `tenant.ordering_toggled` | admin, dunning | Storefront/bot askı modu; `branch.settings_changed`; işletmeye bildirim |
| `sms.quota_threshold` | `sms.send` işi | %80 ve %100'de `notifications` (`sms_quota_warning` / `sms_quota_exceeded`); SMS kesilmez |
| `invoice.paid` **[Faz 2]** | PSP webhook'u, admin | Paraşüt e-Fatura/e-Arşiv |

### 7.2 Outbox konuları → kuyruklar

| `topic` | Kuyruk ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5) | `dedupe_key` | Not |
|---|---|---|---|
...
| `order.finalize_rejection` | `notify` | `order:{id}:reject:{n}` | `available_at = rejection_scheduled_at` (+30 sn); geri almada `cancelled` |
| `tenant.health_score` / `tenant.value_report` / `canary.tenant` | `cron` | `health:{date}` · `value:{tenant}:{month}` · `canary:{branch}:{slot}` | Günlük sağlık skoru (06:00'a kadar), aylık değer raporu (ayın 1'i), tenant canary (15 dk) |
...
| `integration.push` **[Faz 2]** | `integrations` (Faz 2; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5 kanonik listesinde, D06 §4.5) | `integration:{provider}:{order_id}:{event}` | POS aktarımı (SambaPOS/Adisyo, F2-05); `external_refs` UK çift aktarımı engeller; hata sipariş akışını durdurmaz |
...
| Takip linki (`/t/{token}`) ve sayfadaki kişisel alanlar | Teslimden (ret/iptalde final durumdan) 7 gün ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7; D08 §2.8 satır 5) | Link 410 döner (`tracking_expires_at`); kişisel alanlar gösterilmez. Siparişe bağlı ön bilgilendirme ve sözleşme sürümü kişisel veri içermeyen kalıcı `legal_documents.url` adresinde kalır | `retention.tracking_pages` |
...
| Kapanan tenant'ın tüm verisi | Gönüllü iptal: dönem sonu + 30 gün; dunning: G+75 (30 günlük pencere G+45 ön bildirimiyle başlar); deneme: D+90 (D08 §2.8 satır 12) | Silme; bizim faturalarımız kalır; yedeklerden 35 günde düşer | `retention.tenant_offboarding` |
...
| `analytics_events` (PII yok) | 90 gün (`received_at`) | Aylık partition düşürme | `retention.analytics` (D08 §2.8 satır 20) |
| `legal_acceptances`, `invoices`, `payments_subscription` | 10 yıl | — | — |
| `retention_runs`, `data_subject_requests`, `content_takedowns` | ≥ 3 yıl | — | — |
| `tenant_lifecycle_events`, `tenant_onboarding_steps`, `admin_notes`, `admin_tasks`, `tenant_value_reports`, `marketplace_declarations` | Tenant satırıyla (kapanışta silinir; lifecycle olayları anonim metrik olarak kalabilir [T]) | Silme | `retention.tenant_offboarding` |
| `tenant_health_scores`, `incidents`, `incident_tenants` | 2 yıl | Silme | `retention.technical` |
| `data_breach_incidents` | ≥ 5 yıl [T] (D08 ile teyit) | — | — |
| `abuse_blocklist` | `expires_at` (kısa süreli) | Silme | `retention.technical` |
| Teknik: `wa_webhook_events`, `branch_events`, `outbox` (terminal), `print_jobs`, `storefront_link_tokens`, `otp_verifications` | 30 gün | Partition düşürme / silme | `retention.technical` (D08 §2.8 satır 19) |
| Trafik ve erişim logları (yer sağlayıcı; storefront/panel/API erişim kayıtları, IP ve zaman) | 1 yıl (kayıt; 5651 m.5, kesin süre teyit edilmeli; D08 §2.8 satır 10) | Silme | `retention.access_logs` |
| Teknik: `idempotency_keys` 24 sa · `order_verification_codes` 7 gün · `notifications`, `sms_messages` 90 gün (telefon maskelenir) · canary siparişleri ack sonrası / en geç 10 dk (D06 §7.10) · `devices` iptal + 90 gün · DSR dışa aktarım dosyası 7 gün | | Silme | `retention.technical` |
...
- **CI:** boş DB'ye migrate → `test` seed → tenant yalıtım paketi (katalog, fail-closed, IDOR) → şema lint'i. Lint şunları kontrol eder: `pii:` yorumlu her kolon kayıtta var mı, tenant tablosunda bileşik FK var mı, müşteri, sipariş ve mesaj tablolarında sağlık amaçlı kolon adı (`allergy`, `medical`, `health` vb.) yok mu (`products.allergens` ve işletme sağlık skoru `tenants.health_*` / `tenant_health_scores` izin listesindedir), `packages/core/events.ts` analitik şemalarında `pii:` kayıtlı bir alan adı geçiyor mu (`analytics_events`, §3.5).
- **Seed profilleri** (`pnpm db:seed --profile=…`, deterministik UUIDv5 kimlikler):
  - `platform` (tüm ortamlar, prod dahil): `plans` + `plan_features`, 01.10.2026 `wa_rate_cards` satırı, `fx_rates` (48,4), `legal_documents` ilk sürüm yer tutucuları, feature flag'ler ve kanonik kill-switch'ler (`signup_open`, `wa_onboarding`, `campaigns_global`, `llm_parsing`, `sms_fallback`; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4), `subprocessors` envanteri, ilk `platform_owner`, iç `platform` tenant'ı (platform WABA'sı ve destek gelen kutusu, §3.4; [00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §4 "Destek hattı ve P1").
  - `test`: yalıtım testleri için iki tenant (A/B), her rolden kullanıcı, fixture siparişleri; WhatsApp `transport = 'mock'`.
  - `demo` (dev/staging): **"Lezzet Dürüm Kadıköy"** (`slug = lezzet-demo`, `is_demo = true`, `lifecycle_stage = active`, `onboarding_step = live`). 1 şube (Caferağa); 5 kategori, ~24 ürün, 5 seçenek grubu (Porsiyon, Ekmek, Ekstralar, Çıkarılacaklar, Acı), eş adlar ve 1 alkollü örnek ürün (`wa_restricted`). 3 bölge (Caferağa 20 TL / 250 TL, Moda 25 / 300, Fenerbahçe 30 / 350). Saatler 11:00–02:00 ve 1 özel gün. Her rolden kullanıcı (`owner@lezzet-demo.test` …), 2 kurye, 2 cihaz, tarayıcı yazıcısı. 8 müşteri (sahte BSUID `TR.DEMO0001…`, telefon `+90555000000x`). Son 14 güne yayılmış 40 sipariş (Faz 1 kanalları ve her durum; 1 açık `new`, 1 `awaiting_customer`, 1 açık iptal talebi, 1 `onboarding_test`, 1 canary). Sohbetler, niyet etiketli mesajlar, maliyet defteri satırları, 3 değerlendirme, 1 pazaryeri beyanı, 14 günlük sağlık skoru.
...
| # | Konu | Öneri / durum |
|---|---|---|
| 1 | **Tablo adları.** | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5): çoğul snake_case; 02 ve 06 bu dokümandaki adlara (`wa_webhook_events`, `wa_templates`, `wa_pending_statuses`, `product_aliases`, `retention_runs`…) hizalandı. Adres alanları İngilizce (`district`, `directions`…). |
| 2 | **API yolları.** | Hizalandı: tüm yollar `/api/v1/…` (panel, storefront, admin) ve `api…/v1/…` (ajan, açık API). |
| 3 | **Ret geri alma.** | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7): 30 sn "bekleyen ret", `orders.rejection_scheduled_at` + iptal edilebilir gecikmeli iş; `rejected → new` yok (§4.1). |
| 4 | **Debounce ve Akış B.** | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7): debounce yalnız Akış A'da; Akış B'de kod mesajına "alındı" anında gider (§4.1). |
| 5 | **AI buton seti.** | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §7, §10): kanonik set [Onayla] [Düzenle] [İptal], "ödeme yükümlülüğü doğar" ibaresi mesaj gövdesinde; buton kimlikleri `order:{id}:confirm\|edit\|cancel` (§4.2). |
| 6 | **Deneme bitişi.** | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §9): 14 gün → 3 gün uyarı bandı → D+3 askı → 90 gün içinde plan seçilirse veri döner → D+90 silme (§4.3). |
...
| 24 | **Marka alanlarının adlandırması** (`tenants`/`branches`: `brand_color`, `brand_palette`, `logo_url`, `cover_url`). [12](12-marka-tasarim-ve-kullanilabilirlik.md) §13 #3 `logo_image_id`/`cover_image_id` öneriyordu; ürün görselinde `image_key` (R2 anahtarı) kullanılıyor. | Bu turda görev tanımındaki `*_url` adları uygulandı. Faz 1 ilk sprintinde tek desen seçilmeli (öneri: R2 anahtarı saklanır, URL türetilir); seçim 12 ve D04 §7.2 ile birlikte güncellenir. |
| 25 | **Ürün analitiğinin hukuki zemini:** `analytics_events` PII taşımaz, ancak `sessionStorage`'daki oturum kimliği çerez rehberindeki "benzeri teknolojiler" kapsamında olabilir. | Avukat teyidi ([13](13-varsayim-ve-teyit-kaydi.md) V-006, D08 §2.13). Olumsuzsa storefront olayları rıza paneline bağlanır ya da oturum kimliği kaldırılıp yalnız sayfa içi toplu sayım yapılır. |
| 26 | **Sürüm kaydı ve asgari sürüm** (`app_releases`, `app_version_unsupported`): HTTP kodu (426) ve nabız yanıtındaki hedef sürüm alanları [T]. | D06 §16.7 ile S1–S3'te kesinleşir. |
| 27 | **Entegrasyon kuyruğu ve `integration.push` konusu** (Faz 2). | **Karara bağlandı** ([00-kararlar-ve-sozluk.md](00-kararlar-ve-sozluk.md) §5): `integrations` kuyruğu Faz 2 POS/entegrasyon işleri için kanonik listeye eklendi; §7.2 buna hizalandı (D06 §4.5, §18 #24). |
| 28 | **Çeviri tabloları** (Faz 3): `product_translations` tanımlandı; kategori ve seçenek adları için desen açık. | Faz 3 tasarımında; tek tablo (`entity_type`) mı ayrı tablolar mı kararı [T]. |
| 29 | **Dikey uyarlama alanları** (§3.8: `customer_deposits`/`deposit_movements`, `order_custom_forms`/`order_attachments`, `products.unit`/`weight_step` + stok): taslak adlar ve kurallar. | F2-16 (su bayi, pastane) ve F3-11 (market) tasarımında kesinleşir ([09](09-yol-haritasi-ve-sprint-plani.md) §8.2.1). Açık: depozitonun KDV/muhasebe niteliği (mali müşavir), pasta formu görselinin saklama süresi (D08 §2.8'e satır), tartılı üründe tahmini tutar kuralı (D08) [T]. |
