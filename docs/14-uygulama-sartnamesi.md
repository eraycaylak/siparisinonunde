...
PLATFORM_WA_PROVIDER=mock                   # platform numarası: uyarılar + ortak numara (00 §12a madde 8)
PLATFORM_WA_API_KEY=
PLATFORM_WA_PHONE_NUMBER_ID=                # cloud: platform numarasının phone_number_id'si
PLATFORM_WA_DISPLAY_PHONE=+905550000000     # ortak numara E.164 (QR, wa.me, Akış B); mock dışında üretimde zorunlu
PLATFORM_WA_WEBHOOK_TOKEN=                  # ortak webhook: /api/v1/webhooks/wa/shared/<belirteç>; mock dışında üretimde zorunlu (≥16)
...
|---|---|
| `tenants` | `name, slug (unique), legal_name, tax_no, phone, lifecycle_stage, suspension_reason, ordering_enabled (bool, default true), sms_fallback_enabled (default true), brand_color, logo_url, cover_url, marketplace_commission_bp (tasarruf raporu için, default 2500), plan_code ('esnaf'|'pro'|'zincir'), trial_ends_at, live_at, web_live_at, wa_mode ('shared'|'own', default 'shared'), wa_code (dükkan kodu: A–Z0–9, 3–12, en az bir harf — yalnız rakam sipariş numarasıyla karışır; unique null; §8.1)` (0900, 0901) |
...
| `order_verification_codes` | `order_id, code (6 hane, karışmayan harf/rakam), expires_at (30 dk), used_at`; bekleyen kod (`used_at is null`) **tüm işletmelerde tekil** (0900; ortak numara yönlendiricisi kodu işletme bilmeden bulur) |
...
| `wa_accounts` | `tenant_id, branch_id, provider ('mock'|'cloud'|'d360'|'shared'), display_phone, phone_number_id, waba_id, api_key_enc, webhook_token (unique, URL'de), status ('connected'|'disconnected'|'error'), last_webhook_at, last_error`. `shared` (0900): ortak numara işletmesinin satırı — kimlik bilgisi yok, `display_phone` = platform numarası (açılışta `PLATFORM_WA_DISPLAY_PHONE`'dan eşitlenir), kendi webhook adresi yok (§8.1) |
...
| `push_subscriptions` | Web Push cihaz aboneliği (0700): `tenant_id, branch_id (null = üyeliğin tüm şubeleri), user_id, session_id (oturum silinince abonelik de silinir), endpoint (unique), p256dh, auth, user_agent, last_success_at, failed_at, last_error, disabled_at (itme servisi 404/410)` |
| `branch_panel_presence` | Panel çevrimdışı dedektörü (0700): `branch_id pk, tenant_id, last_seen_at (sipariş ekranı SSE akışı açıkken dakikada bir), offline_alerted_at (şube başına 60 dk'da 1 uyarı)` |
...
| `retention_runs` | İmha tutanağı (0800; 08 §2.8 kabul kriterleri): `job_name, tenant_id null (tenant bazlı adımda dolu), affected_count, started_at, finished_at, duration_ms, error null`. `cron.retention`'ın her adımı bir satır, koşu sonunda `job_name = 'cron.retention'` özet satırı yazar. ≥ 3 yıl, saklama işi silmez; tenant silinince `tenant_id` NULL olur. (07 §3.6'daki hedef şemadaki `job`/`affected jsonb`/`status` yerine Faz 1'de adım başına tek sayı) |
| `shared_wa_routes` | **Küresel** (tenant_id yok; 0900): ortak numaraya yazan kişinin yönlendirme kaydı — `wa_bsuid (unique null), phone_e164 (unique null), current_tenant_id, recent_tenant_ids uuid[] (en yeni başta, ≤ 5), last_routed_at, last_inbound_at, last_picker_at`. Ad/adres/sipariş tutmaz; 24 ay hareketsizlikte silinir (`retention.shared_wa_routes`); işletmenin KVKK müşteri silmesi o işletmeyi kayıttan çıkarır |
| `shared_wa_messages` | **Küresel** (0900): hiçbir işletmenin sohbetine girmeyen platform mesajları (dükkan seçici P01–P05 ve dükkana yönlenmemiş gelen mesaj) — `route_id, direction, wamid (unique), kind, body, payload (code, spec, to), status, error_code`; 30 gün (`retention.technical.shared_wa_messages`) |
...
**Seed (dev):** platform admin `admin@siparisinonunde.local / admin1234`; demo işletme **"Bozok Pide Salonu"** (`bozok-pide`, Yozgat Merkez), sahip `demo@siparisinonunde.local / demo1234`, kasiyer `kasa@…/kasa1234`, kurye `kurye@…/kurye1234`; menü (Pideler, Lahmacun, Kebaplar, Çorbalar, İçecekler, Tatlılar; seçenek grupları: "Porsiyon" (tam/1,5 +%), "Ekstralar" (kaşar +, yumurta +, max 3), "Acı" (acılı/acısız, zorunlu 1)); saatler 10:00–23:30; 3 bölge (mahalle listesi — **örnek**; gerçek mahalle adları işletmece düzenlenir); ortak numara (`shared`, dükkan kodu `BOZOK`; mock platform numarası `+90 555 000 00 00`); birkaç geçmiş sipariş. İkinci demo işletme **"Çamlık Döner"** (`camlik-doner`, Yozgat Merkez, kod `DONER`, sahip `doner@siparisinonunde.local / doner1234`): 8 ürünlük menü (dürüm, porsiyon, içecek; "Acı" seçeneği), aynı saat ve bölge deseni, ortak numara. Seed her işletmeyi ayrı ekler (var olan atlanır); demo kodu başka işletmedeyse boştaki ilk aday kullanılır (`DONER2`…), Döner sahibinin e-postası başka hesaptaysa Döner atlanır — dev dağıtımı seed'i her açılışta çalıştırdığından çakışma açılışı durdurmaz.
...
| Metot | Yol | Açıklama |
|---|---|---|
| GET | `/store/:slug` | İşletme + şube vitrini: `{tenant:{name,slug,brandColor,logoUrl,coverUrl,phone,whatsappPhone,whatsappLink,whatsappMode ('shared'|'own'|null),whatsappCode,whatsappPrefillText}, branch:{id,name,address,orderingState ('open'|'busy'|'paused'|'closed'), nextOpenAt?, acceptsDelivery, acceptsPickup, paymentMethods, mealCardBrands, prepMinutes}, zones:[{id,name,kind,neighborhoods,feeKurus,minOrderKurus,etaMinutes}], categories:[{id,name,products:[{id,name,description,priceKurus,imageUrl,soldOut,optionGroups:[{id,name,minSelect,maxSelect,options:[{id,name,priceDeltaKurus}]}]}]}], legal:{…künye} }` |
...
| GET | `/store/track/:token` | Takip: `{order:{number,status,statusLabel,timeline[],items[],totals,fulfillmentType,etaAt?,canCancel,canRequestCancel,review?}, business:{name,phone,waPhone,waLink}, expired:false}` (`waLink`: ortak numarada `#KOD`'lu bağlantı); süresi dolmuşsa 410 + kişisel veri yok |
...
| WhatsApp | `GET /panel/whatsapp` (owner; sözleşme `core/contracts/whatsapp.ts`: `{mode ('shared'|'own'), code, shared:{code, displayName, displayPhone, displayPhoneFormatted, prefillText, waLink, qrSvg, selectable, selectableReason, canEditCode:false}|null, account (ortak numarada provider 'shared', webhookUrl null), health, smsFallback, providers}`; eksik ortak numara satırı kendiliğinden tamamlanır) · `GET /panel/whatsapp/qr?format=svg|png` (owner, manager; QR indirme, `Content-Disposition: attachment`) · ortak numarada aşağıdaki PUT/disconnect/rotate uçları 409 `wa_shared_mode` · `PUT /panel/whatsapp {provider ('mock'|'cloud'|'d360'), displayPhone, phoneNumberId?, wabaId?, apiKey?}` (owner) · `POST /panel/whatsapp/test` · `POST /panel/whatsapp/disconnect` (owner; `status='disconnected'`, API anahtarı ve numara kimliği silinir, webhook belirteci yenilenir, satır sohbet geçmişi için kalır; vitrin "WhatsApp'tan yaz"ı gizler, Akış B açıksa SMS OTP'ye düşer) · `POST /panel/whatsapp/rotate-webhook-token` (owner; yeni gizli webhook adresi → `{…, webhookUrl}`, eski adres hemen 404) |
...
**Bildirim (Web Push, 00 §10 alarm t=0; 04 §4.5).** Roller: kurye hariç panel rolleri. Destek oturumu (impersonation) yazma uçlarını kullanamaz (403 `impersonation_not_allowed`; salt-okunurda `read_only_session`).

| Uç nokta | Açıklama |
|---|---|
| `GET /panel/push/public-key` | `{enabled, publicKey}`; VAPID tanımlı değilse `enabled=false, publicKey=null` |
| `POST /panel/push/subscribe` | `{endpoint, expirationTime?, keys:{p256dh, auth}}` (tarayıcının `PushSubscription.toJSON()`'u). Endpoint'e göre ekle/güncelle; kullanıcı + işletme + (üyelik şubeye kısıtlıysa) şube + oturuma bağlanır; kapatılmış abonelik yeniden açılır. Yalnız bilinen itme servisleri kabul edilir (`fcm.googleapis.com`, `push.services.mozilla.com`, `push.apple.com`, `notify.windows.com`, `android.googleapis.com`; https) → aksi 400 |
| `POST /panel/push/unsubscribe` | `{endpoint}` → `{removed}`; yalnız çağıranın bu işletmedeki aboneliği silinir |
| `POST /panel/push/test` | `{endpoint}` → `{sent, reason}`; yalnız çağıranın bu cihazdaki aboneliğine test bildirimi (kullanıcı başına dakikada 5) |

Web: service worker `apps/web/public/panel-sw.js` (kapsam `/panel/`, `updateViaCache: 'none'`; yalnız `push`, `notificationclick`, `pushsubscriptionchange`; fetch yakalamaz, önbellek yok). İzin ve abonelik "Siparişleri almaya başla" ve "Sesi aç" dokunuşunda istenir; `/panel/ayarlar/bildirimler/cihaz` ("Bu cihazda bildirimler") durum, aç/kapa (cihaz başına tercih), test bildirimi ve iOS yardımı gösterir. Panel açılışında izinli cihazın mevcut aboneliği sessizce oturuma yeniden bağlanır.

### 6.4 Admin (`/api/v1/admin`, platform oturumu)
`GET /admin/overview` (işletme sayıları lifecycle'a göre, bugünkü sipariş, açık alarm, başarısız iş, saklama işi sağlığı) · `GET /admin/tenants?q=&stage=` · `GET/PATCH /admin/tenants/:id` (plan, lifecycle, suspension_reason, ordering_enabled; `waCode` (normalize edilir, tekil → 409 `wa_code_taken`, biçim/komut/sipariş koduna benzeyen → 400 `invalid_wa_code`) ve `waMode` ('shared'|'own'; hesap satırı çevrilir) — izin `tenants:whatsapp` (PO, PA); detayda `tenant.waMode, waCode, sharedWaLink`, listede `waMode, waCode`) · `GET /admin/tenants/:id/orders` · `POST /admin/tenants/:id/impersonate {reason}` → salt-okunur 30 dk oturum · `POST /admin/impersonation/end` · CRUD `/admin/tenants/:id/notes` · `GET /admin/whatsapp` (tüm hesapların sağlığı; `sharedNumber`: ortak numaranın yapılandırma özeti — numara, sağlayıcı, maskeli webhook adresi (belirtecin son 4 karakteri), son ortak webhook olayı, ortak numaradaki/seçilebilir işletme sayısı, son 24 saat platform mesajları, `health` ve Türkçe `problems[]`) · `GET /admin/jobs?status=failed` · `POST /admin/jobs/:id/retry` · `GET/PATCH /admin/flags` · `GET /admin/leads` · `PATCH /admin/leads/:id` · `GET /admin/audit?tenantId=`

### 6.5 Webhook ve geliştirme
- `GET /api/v1/webhooks/wa/:webhookToken` — Meta doğrulama (`hub.challenge`).
- `POST /api/v1/webhooks/wa/:webhookToken` — Cloud API/360dialog biçimli olay; `WA_APP_SECRET` varsa `X-Hub-Signature-256` doğrulanır; ham olay `wa_webhook_events`e yazılır, **hemen 200**, işleme `wa-inbound` kuyruğunda. Ortak numara satırının (`provider='shared'`) işletmeye özel adresi yoktur (404).
- `GET/POST /api/v1/webhooks/wa/shared/:token` — **ortak numara** (§8.1): `PLATFORM_WA_WEBHOOK_TOKEN` (tanımsızsa 404); aynı doğrulama/imza (`PLATFORM_WA_PROVIDER=d360` ise imza denetlenmez)/ham olay (`provider='shared'`, hesapsız)/hemen 200 kuralları; dükkan seçimi `wa.process_inbound` işinde.
- `POST /api/v1/public/leads` — demo talebi/hesaplayıcı lead'i.
- `DEV_TOOLS=1` iken: `GET /api/v1/dev/wa/accounts` (`items[]` + `mode`, `waCode`; `sharedNumber:{displayName, displayPhone, displayPhoneFormatted, shops:[{tenantId, tenantName, slug, waAccountId, code, prefillText, waLink, selectable}]}`), `POST /api/v1/dev/wa/inbound {waAccountId? | shared:true, from:{phone,name,bsuid?}, message:{type:'text'|'button_reply'|'list_reply'|'location', …}, contextWamid?}` (Cloud API webhook yükü üretip aynı işlem hattına sokar; `shared:true` ya da ortak numara işletmesinin hesabı → ortak numara hattı; hesap kimliğiyle yazmak müşterinin o dükkanın QR'ından geldiği sayılır — etkin dükkanı yoksa yönlendirme o dükkana hazırlanır; `shared:true` kodsuz gerçek deneyim), `GET /api/v1/dev/wa/thread?waAccountId=&phone=` ya da `?shared=1&phone=` (mock giden + gelen mesajlar; ortak numarada müşterinin tek sohbeti: tüm dükkanlar + platform seçicisi, mesajda `tenantName`, `platform`, `brand`, `wamid` (simülatör buton/liste yanıtında `contextWamid` olarak gönderir); yanıtta `shared:true`, `route`), `POST /api/v1/dev/wa/echo` (işletme telefonundan yazılmış mesaj — coexistence echo; ortak numarada 409 `wa_shared_mode`), `GET /api/v1/dev/sms` (mock SMS'ler).
...
| `wa.send` | wa-outbound | outbox | sağlayıcıya gönder, `messages.status` (ortak numara satırı platform sağlayıcısıyla, numara hız kovası ortak) |
| `wa.send_shared` | wa-outbound | ortak numara yönlendiricisi | platform düzeyi mesajı (dükkan seçici P01–P05) platform numarasından gönderir, `shared_wa_messages.status` |
...
| `push.send` | notify | sipariş `new` olunca (telefon siparişi ve canary hariç; tekillik `push:new_order:<orderId>`, en çok 3 deneme) | 00 §10 t=0 Web Push: şubeye erişen owner/manager/cashier/kitchen kullanıcılarının etkin, oturumu geçerli aboneliklerine `{kind, title:"Yeni sipariş #1051", body:"3 ürün · 245,00 TL", url:"/panel", tag:orderId}` (müşteri adı/telefonu/adresi yok; mutfakta tutar yok; kurulum testinde "TEST #"). Sipariş artık `new` değilse gönderilmez. 404/410 → abonelik `disabled_at`; 5xx/408/429/ağ → o abonelik için `subscriptionId`'li ayrı `push.send` (iş kurallarıyla geri çekilme); diğer 4xx → yalnız kayıt. Ana şeritte, istek başına 10 sn üst süre |
| `cron.panel_presence` | cron | dakikada bir | Panel çevrimdışı dedektörü (06 §7.7): sipariş alan (çalışma saati içinde, duraklatılmamış, `ordering_enabled`, `web_live_at` dolu; lead/onboarding/read_only/suspended/churned ve demo hariç) şubede sipariş ekranı 5 dk'dır görülmüyorsa ya da açılıştan beri hiç görülmeyip açılış ≥ 10 dk önceyse → `platform.alert {kind:'panel_offline', minutes}` (şube başına 60 dk'da en çok 1). Varlığı yalnız owner/manager/cashier SSE akışı yazar (mutfak ve destek oturumu yazmaz) |
| `cron.retention` | cron | günlük 03:00 | 08 §2.8 silme ve anonimleştirme işleri (`apps/api/src/jobs/system`). Her adım idempotenttir, sonucunu `retention_runs`'a yazar; hatalı adım diğerlerini durdurmaz, koşu sonunda iş hata verip yeniden denenir. Adımlar (`job_name`): `retention.technical.*` (branch_events 7 gün; wa_webhook_events, otp_verifications, bitmiş jobs, storefront_link_tokens ve `shared_wa_messages` 30 gün; courier_login_links 7 gün; süresi dolan sessions; **sms_messages 90 günde silinmez, `to_phone` `maskPhone` ile maskelenir**, durum/amaç/kota kalır) · `retention.locations`, `retention.media` 30 gün (belge/video/çıkartma `system` mesajlarının medya kimliği dahil) · `retention.order_notes` (+ `.items`) final durumdan 30 gün · `retention.wa_messages` (+ `.previews`) 6 ay · `retention.customer_inactive`: son sipariş (müşteri kaydı + aynı telefonla verilen siparişler) ve son gelen mesajdan 24 ay (`CUSTOMER_INACTIVE_MONTHS`; işletmenin 6–24 ay ayarı için tenant kolonu henüz yok) hareketsiz, açık siparişi olmayan müşteri elle KVKK silmesiyle aynı `eraseCustomer` ile anonimleşir (aktör sistem: `customer_erasures.erased_by_user_id` boş, `audit_log` `customer.erase` `data.actorType = 'system'`); tenant başına bir tutanak satırı; 08'deki "haftalık" yerine günlük koşuda · `retention.access_logs.orders` / `.legal_acceptances`: sipariş onayındaki `confirmation_ip`/`confirmation_user_agent` ve son müşteri belge kabulündeki `ip`/`user_agent` 1 yıl sonra boşaltılır, kayıt kalır · `retention.shared_wa_routes`: ortak numara yönlendirme kaydı son gelen mesaj/yönlendirmeden 24 ay sonra silinir · `retention.audit` 2 yıl · `retention.leads` son temastan (`updated_at`) 12 ay. Erişim logu 1 yıl Caddy `roll_keep_for` ile. Henüz yok: `retention.users` (hesap kapanışı kavramı yok), `retention.tenant_offboarding`, `retention.analytics` (tablo yok). Admin `GET /admin/overview` → `retention {lastRunAt, error, stale, alert}`: son koşu 48 saatten eskiyse ya da hatalıysa alarm |
...
- **Model:** ortak numara işletmesinin varsayılan şubesinde `provider='shared'` bir `wa_accounts` satırı vardır (kayıtta, admin mod değişiminde ve seed'de açılır; `ensureSharedWaAccount`). Konuşma, mesaj, müşteri, outbox ve bildirim hattı bu satır üzerinden **işletme başına aynen** çalışır; gönderim `PLATFORM_WA_*` sağlayıcısıyla yapılır (`providerForAccount`, `platformAccountRef`). Platform uyarıları aynı numaradır. `tenants.wa_mode` yöneticinin seçtiği moddur; `applyWaMode` satırı birlikte çevirir (`own`: satır `disconnected`, işletme panelden kendi numarasını bağlar).
- **Gelen mesaj:** ortak webhook ham olayı hesapsız kaydeder; `wa.process_inbound` → `services/messaging/shared-router.ts`: kişi başına danışma kilidi (`wa_shared:<anahtar>`), yönlendirme kaydı (`shared_wa_routes`, önce BSUID sonra telefon), karar, sonra seçilen işletmenin kendi motoru (`handleInboundMessage`; seçimde `selected` → soğuma beklemeden durum kartı / kapalı-duraklatılmış bilgisi / tam karşılama). Karar sırası:
  - Tekrar teslim: aynı `wamid` önceki kararda dükkana (`messages`) ya da platforma (`shared_wa_messages`, gelen) yazılmışsa kişi kilidi altında `duplicate` sayılır; arada müşteri dükkan seçmiş olsa da eski mesaj o dükkana gitmez.
  - `request_welcome` (sohbet ilk kez açıldı) sessizdir: kayıt `shared_wa_messages`'a düşer, seçici gönderilmez (QR'ın ön-dolu `#KOD` mesajı hemen ardından dükkanı seçer; seçici soğuması da başlamaz).
  0. "DUR"/"BAŞLAT" → alıntılanan (yanıtlanan) dükkan mesajının işletmesi; alıntı yoksa kişiye ortak numaradan en son yazan işletme (güncel + son dükkanlar arasında); o da yoksa güncel dükkan (süre sınırı yok); dükkan yoksa yanıt yok. Güncel dükkan **değişmez** (başka dükkanın mesajına verilen "DUR" etkin siparişin dükkanını bozmaz).
  1. Buton/liste yanıtı: `shop:{id}` → o dükkan (seçilemiyorsa bilgi + seçici); `shops:list`/`shops:page:{n}` → liste; `shops:m:{liste}:{n}` → ad eşleşmesi listesinin n. sayfası (eşleşenler ilk listenin `shared_wa_messages.payload.matchIds`'inde; yalnız aynı kişinin listesi, yoksa tüm liste); sipariş kimlikli butonlar (`review:`, `review_reason:`, `wait:`, `cancel:`, `keep:`, `order:`) → siparişin işletmesi; diğerleri yanıtlanan mesajın (`context.id` → `messages.wamid`) işletmesi.
  2. Akış B sipariş kodu → kodun işletmesi (önce bekleyen kod, son 7 gün); bulunamazsa sıradaki kurallar (seçicide "kod bulunamadı" notu).
  3. `#KOD` ya da — etkin (24 sa) dükkan yokken — mesajın tamamı bir kod (Türkçe harf/küçük harf normalize) → dükkan seçilir (etkin oturumda "pide" gibi yalın bir sözcük başka dükkanın koduna denk gelse de dükkan değişmez); kendi numaralı dükkan → numarası ve wa.me bağlantısı (P03); seçilemeyen dükkan → P04. Bilinmeyen `#KOD` yok sayılır; harfsiz `#1047` sipariş numarasıdır (kod en az bir harf içerir). Kod **zaten etkin güncel dükkanın** koduysa (takip sayfası, fiş ve vitrin bağlantıları hep `#KOD` taşır) yeni seçim sayılmaz: motorun olağan sırası ve soğumaları geçerlidir ("yetkili", iptal, SSS; aynı QR'ı tekrar okutmak 30 dk içinde sessiz, sonra kısa karşılama). Seçimde de motor önce "yetkili" isteğini, açık siparişte iptal isteğini işler.
  4. Komutlar: "dükkanlar", "liste" → tüm liste; "dükkan", "değiştir", "başka dükkan" → seçici.
  5. Yanıtlanan (alıntılanan) dükkan mesajı → o dükkan.
  6. Son 24 saatte yönlendirilmiş güncel dükkan → devam.
  7. Dükkan adı (`shared-match.ts`): adın tamamı ya da tüm anlamlı sözcükler eşleşirse güçlü; kısa mesajda (≤ 3 sözcük) tek sözcük de sayılır. Tek eşleşme seçilir, birden çok eşleşme listelenir.
  8. Seçici (kendiliğinden; kişi başına 60 sn'de en çok 1): son dükkanlar varsa en çok 2'si + (başka seçilebilir dükkan varsa) "Diğer dükkanlar" reply butonu (P01; buton başlıkları mesajda tekildir — 20 karakterde aynılaşan adlara dükkan kodu, kod yoksa sıra numarası eklenir); yoksa tüm liste (P02). Seçilebilir dükkan yoksa P05.
- **Seçilebilir dükkan:** `wa_mode='shared'`, bağlı `shared` satırı, `web_live_at` dolu, `ordering_enabled`, askı sebebi yok, aşama `suspended`/`read_only`/`churned` değil.
- **Liste mesajı:** bölümler "İlçe, Şehir" başlıklı; mesaj başına en çok 10 satır (teyit edilmeli): 10'dan fazla dükkanda sayfa başına 9 dükkan + "Diğer dükkanlar" (`shops:page:{n+1}`; son sayfada "Listenin başı"). Satır başlığı ≤ 24, açıklama mahalle/ilçe.
- **Platform mesajları** (P01–P05, `core/messages/tr.ts` `SHARED_PICKER_TEXTS`) hiçbir işletmenin sohbetine girmez: `shared_wa_messages` + `wa.send_shared`; dükkana yönlenmemiş gelen mesaj da orada kalır (wamid tekil → tekrar teslimde yanıt yinelenmez).
- **Marka:** ortak numaradaki konuşmanın her serbest mesajı `queueOutbound`'da dükkan adını alır: metinde `*Dükkan adı*` ilk satırı, etkileşimli mesajda başlık (header; konum isteğinde gövde satırı); `messages.body` kalın satırla başlar, `payload.brand` = ad. Panelden elle yanıt da aynı. Pencere dışı şablonların hepsi `{isletme}` değişkenini taşır (birim testli). Mesaj bütçesi ve debounce kuralları değişmez.
- **Akış B:** vitrindeki "WhatsApp ile onayla" bağlantısı ortak numaraya (`wa.me/<ortak numara>?text=Sipariş kodu: …`); bekleyen kod küresel tekil olduğundan doğru işletme bulunur. Vitrinde ve takip sayfasında "WhatsApp'tan yaz" `#KOD`'lu bağlantıdır; fişteki WhatsApp satırı kodu da yazar.
- **Dükkandan başlayan mesaj** (ör. telefon siparişinin durum şablonu): gönderim başarısında (`rememberSharedTenant`) kişinin etkin (24 sa) güncel dükkanı yoksa bu dükkan güncel olur — müşterinin yanıtı o dükkana gider; etkin başka dükkan varsa oturum bozulmaz, dükkan yalnız son dükkanlara eklenir.
- **Durum olayları:** `wamid` işletme mesajındaysa o işletmenin hesabıyla (monoton), platform mesajındaysa `shared_wa_messages`. Echo yok sayılır.
- **Webhook koruması:** yanlış belirteçli istekler IP başına ayrı küçük kovayla sınırlanır (60/dk, sonra 429) ve 404 alır; ortak kova (12.000/dk) yalnız doğru belirteçli istekle harcanır. Belirteç API ve Caddy erişim loglarında maskelenir (`/webhooks/wa/shared/***`). `PLATFORM_WA_PROVIDER=cloud` iken `WA_APP_SECRET` zorunludur (imzasız olay kabul edilmez).
- **Panel test mesajı:** ortak numarada yalnız hesaptaki telefona ya da son 24 saatte o işletmeye yazmış müşteriye (`400 test_recipient_not_allowed`); işletme başına 10 dakikada en çok 3 (429). Kendi numaralı işletmede değişmez.
- **Üretimde taklit sağlayıcı:** `PLATFORM_WA_PROVIDER=mock` ve `PLATFORM_WA_DISPLAY_PHONE` boşken ortak numara gösterim numarası yoktur (geliştirme numarası +90 555 000 00 00 yalnız geliştirme/test ve `DEPLOY_ENV=dev`); vitrin, QR ve Akış B WhatsApp bağlantısı göstermez.
- **Hata:** ortak numarada 190/131042 işletmenin satırını duraklatmaz, işletmeye "bağlantı sorunu" uyarısı gitmez (platform sorunudur; log). Admin WhatsApp sağlığında ortak numara satırı "sessiz" sayılmaz.
...
- **Storefront (`/s/[slug]`):** mobil öncelikli; altbilgideki "WhatsApp'tan yaz" `tenant.whatsappLink`'i kullanır (ortak numarada `#KOD`'lu ön-dolu mesaj), takip sayfasındaki "WhatsApp'tan yaz" ortak numarada mesaja `#KOD` ekler; kapalı/meşgul bandı; kategori sekmeleri; ürün kartı → seçenek sayfası/çekmece (min/max doğrulama); sepet çekmecesi; `/s/[slug]/siparis` checkout (teslim türü, ad, telefon, mahalle seçimi + adres + tarif, ödeme yöntemi, para üstü, çatal-bıçak, not, ön bilgilendirme özeti + "Siparişi onayla" + altında "ödeme yükümlülüğü doğar"); gönderim sonrası Akış B ekranı (WhatsApp ile onayla butonu + kod + "SMS ile doğrula" + durum yoklama); "Son siparişin" kartı; altbilgide "Altyapı: Siparişin Önünde". `?l=` parametresi varsa `POST /session` ile çereze çevrilir ve URL temizlenir.
- **Storefront yasal metinleri (`/s/[slug]/yasal/[doc]`, S-10):** `aydinlatma` (KVKK aydınlatma metni; veri sorumlusu işletme, platform veri işleyen), `on-bilgilendirme` (ön bilgilendirme formu), `mesafeli-satis` (mesafeli satış sözleşmesi; çabuk bozulan gıdada cayma hakkı istisnası, Mesafeli Sözleşmeler Yönetmeliği m.15). Satıcı künyesi `GET /store/:slug` yanıtının `legal` bloğundan dolar; eksik alan köşeli parantezli yer tutucuyla gösterilir (künye eksikken vitrin canlıya alınamaz, 08 §4.7). Sürüm `LEGAL_DOCUMENT_VERSION` (siparişe bağlanan kabul kaydıyla aynı), `noindex`, yazdırılabilir. Bağlantılar: storefront altbilgisi, checkout ön bilgilendirme onayı, takip sayfası altbilgisi (takip yanıtındaki `business.slug`), WhatsApp karşılama mesajındaki aydınlatma satırı (`APP_BASE_URL/s/<slug>/yasal/aydinlatma`).
- **Takip (`/t/[token]`):** durum çizelgesi, tahmini süre, kalemler, iptal/iptal talebi, teslimden sonra 3 butonlu değerlendirme.
- **Panel (`/panel`):** `/panel/giris`, `/panel/kayit` → onboarding sihirbazı (`/panel/kurulum`); canlı ekran `/panel` (sütunlar Yeni / Onaylandı / Hazırlanıyor / Hazır / Yolda; "Vardiyayı başlat" butonu ses kilidini açar ve Wake Lock alır; yeni siparişte döngüsel ses + kırmızı yanıp sönen kart; bağlantı koptu bandı; "Onayla · 20/30/45 dk" hızlı butonları; ret sebep çipleri + 30 sn "Geri al" şeridi; kurye atama; "Gecikme bildir"; fiş yazdır (80 mm CSS, `window.print`)); `/panel/siparisler`, `/panel/telefon-siparisi`, `/panel/menu`, `/panel/sohbetler`, `/panel/musteriler`, `/panel/raporlar`, `/panel/ayarlar/*` (işletme, şube, saatler, bölgeler (mahalle listesi + haritada poligon/yarıçap), ödeme, bildirim/durum mesajları, alarm, WhatsApp, personel, fiş, QR/afiş (QR PNG + A5 afiş yazdırma)), `/panel/kuryeler`. Ayarlar › WhatsApp ortak numarada: "Siparişleriniz ortak Siparişin Önünde numarasından gelir", dükkan kodu (salt okunur; değişiklik için destek), müşteri bağlantısı (kopyala / WhatsApp'ta aç), QR önizleme ve indirme (SVG/PNG), yazdırılabilir A5/A6 masa kartı ("Sipariş vermek için okut", dükkan adı, kod; baskıda tek sayfa), listede görünürlük, test mesajı ve ikincil "Kendi numaranızı bağlayın" bölümü (geçişi platform yapar); kendi numara modunda mevcut bağlantı formu. Kurulum sihirbazının WhatsApp adımı ortak numarada kendiliğinden tamamdır ve QR'ı gösterir. QR ve afiş sayfası ortak numarada QR'ı dükkan kodlu bağlantıyla üretir (numara girilmez). Sohbet balonları WhatsApp biçimini (*kalın*, _italik_, ~üstü çizili~) ve liste bölüm başlıklarını gösterir (mesaj görünümünde liste satırı `section`).
...
- **Admin (`/admin`):** giriş, özet, işletmeler (liste + detay: profil, lifecycle, plan, WhatsApp sağlığı, siparişler, notlar, impersonation; listede ve başlıkta WhatsApp modu + `#KOD`; WhatsApp sekmesinde mod ve dükkan kodu düzenleme — gerekçe zorunlu, kod istemcide core kurallarıyla denetlenir, 409 `wa_code_taken` Türkçe alan hatası), WhatsApp sağlık tablosu (üstte ortak numara kartı), işler/DLQ, bayraklar (kill-switch), lead'ler, audit.
- **Geliştirici simülatörü (`/dev/whatsapp`):** yalnız `DEV_TOOLS=1`; sol: numara seçimi ("Siparişin Önünde · ortak numara" ya da kendi numaralı işletmeler) + müşteri (telefon/ad); orta: WhatsApp benzeri sohbet (gelen/giden balonlar, butonlar ve liste satırları tıklanabilir, CTA URL yeni sekmede açılır); "İşletme telefonundan yaz" (echo; ortak numarada yok) ve SMS kutusu. Ortak numara (§8.1): tek "Siparişin Önünde" sohbeti, dükkan QR'ları (#KOD ön-dolu metni) tek dokunuşla gönderilir; dükkan mesajları dükkan adıyla, seçici mesajları platform adıyla görünür; her balonda yönlendirme etiketi (→ dükkan / dükkan seçici), başlıkta etkin dükkan (24 saat kuralı).
...
- `pnpm e2e` — Playwright (mock WhatsApp): (1) Akış A: simülatörde dükkan QR'ı (#BOZOK) → menü linki → sepet → sipariş → panelde görünür → onayla → simülatörde "onaylandı" mesajı; (2) Akış B: storefront'tan doğrudan sipariş → WhatsApp kodu → `new`; (3) SMS OTP yedeği; (4) ret + geri al; (5) kurye akışı; (6) admin impersonation salt-okunur; (7) kayıt + kurulum sihirbazı; (8) 360 px; (9) ortak numara: iki dükkanın QR'ı (#BOZOK, #DONER), dükkana özel sipariş ve panel yalıtımı, ertesi gün kodsuz mesajda dükkan seçici (son dükkanlar), seçimle doğru dükkana bağlanma; işletme sahibinin WhatsApp sayfası 360 px.
