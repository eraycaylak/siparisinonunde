...
| WhatsApp | `GET /panel/whatsapp` (bağlantı + sağlık) · `PUT /panel/whatsapp {provider, displayPhone, phoneNumberId?, wabaId?, apiKey?}` (owner) · `POST /panel/whatsapp/test` · `POST /panel/whatsapp/disconnect` (owner; `status='disconnected'`, API anahtarı ve numara kimliği silinir, webhook belirteci yenilenir, satır sohbet geçmişi için kalır; vitrin "WhatsApp'tan yaz"ı gizler, Akış B açıksa SMS OTP'ye düşer) · `POST /panel/whatsapp/rotate-webhook-token` (owner; yeni gizli webhook adresi → `{…, webhookUrl}`, eski adres hemen 404) |
...
`GET /admin/overview` (işletme sayıları lifecycle'a göre, bugünkü sipariş, açık alarm, başarısız iş, saklama işi sağlığı) · `GET /admin/tenants?q=&stage=` · `GET/PATCH /admin/tenants/:id` (plan, lifecycle, suspension_reason, ordering_enabled) · `GET /admin/tenants/:id/orders` · `POST /admin/tenants/:id/impersonate {reason}` → salt-okunur 30 dk oturum · `POST /admin/impersonation/end` · CRUD `/admin/tenants/:id/notes` · `GET /admin/whatsapp` (tüm hesapların sağlığı) · `GET /admin/jobs?status=failed` · `POST /admin/jobs/:id/retry` · `GET/PATCH /admin/flags` · `GET /admin/leads` · `PATCH /admin/leads/:id` · `GET /admin/audit?tenantId=`
